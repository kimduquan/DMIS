...
namespace Portal.Portlets
{
    public class AssignWorkflowPortlet : ContentAddNewPortlet
    {
        ...
        protected void AssignWorkflowButton_Click(object sender, EventArgs e)
        {
            ...
            {
                this.ContentView.ContentException = new SecurityException("Only content of Workflow content type can be created with this portlet.");
                return;
            }
            ...
            if (this.ContentView.IsUserInputValid && content.IsValid)
            {
                try
                {
                    ...
                    if (!(allowManualStart || autostartOnPublished || autostartOnCreated || autostartOnChanged))
                    {
                        ...
                        throw new ContentRepository.Storage.InvalidContentException("At least one start option has to be specified.");
                    }
