...
namespace Portal.Portlets.Controls
{
	...
	public class ExportFormItems : WebControl
	{
		...
		protected override void OnInit(EventArgs e)
		{
			...
			tbFrom = new TextBox();
			tbFrom.ID = "tbFrom";
			tbFrom.Text = new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, 1).ToString("yyyy.MM.dd");

			tbTo = new TextBox();
			tbTo.ID = "tbTo";
			tbTo.Text = new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, 1).AddMonths(1).ToString("yyyy.MM.dd");
		...
		private string GetCSV(Form form)
		{
			...
			ContentRepository.Storage.Security.AccessProvider.ChangeToSystemAccount();
			...
			ContentRepository.Storage.Security.AccessProvider.RestoreOriginalUser();
