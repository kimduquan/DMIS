...
<%@ Control Language="C#" AutoEventWireup="false" Inherits="Portal.UI.ContentListViews.ViewFrame" %>

<sn:ContextInfo runat="server" Selector="CurrentContext" UsePortletContext="true" ID="myContext" />

<% 
   var trashbin = ContentRepository.TrashBin.Instance;
...
<div class="sn-listview">
    ...
    <sn:Toolbar runat="server">
        ...
        <sn:ToolbarItemGroup Align="Right" runat="server">
             <% if (this.ContextNode != null && ScenarioManager.GetScenario("Views").GetActions(ContentRepository.Content.Create(this.ContextNode), null).Count() > 0) 
               { %>
            ...
            <sn:ActionMenu runat="server" IconUrl="/Root/Global/images/icons/16/views.png" Scenario="Views" ContextInfoID="myContext"
              ...
              <%= HttpUtility.HtmlEncode(ContentRepository.Content.Create(ViewManager.LoadViewInContext(ContextNode, LoadedViewName)).DisplayName)%>
            </sn:ActionMenu>
             <% } %>
