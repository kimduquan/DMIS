...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                xmlns:snl="sn://Portal.UI.ContentListViews.ListHelper"
  ...
  <xsl:template match="/">
    <![CDATA[<%@ Control Language="C#" AutoEventWireup="false" Inherits="Portal.UI.ContentListViews.ListView" %>
    ...
    <xsl:for-each select="Column">
      <xsl:choose>
        <xsl:when test="contains(@modifiers,'main')">
          ...
          <![CDATA[ <sn:ActionMenu NodePath='<%# Eval("Path") %>' runat="server" Scenario="]]><xsl:value-of select="$mainscenario"/><![CDATA[" IconName="<%# ((ContentRepository.Content)Container.DataItem).Icon %>" OverlayVisible="true" >]]>
          <![CDATA[  <sn:ActionLinkButton runat='server' NodePath='<%# Eval("Path") %>' ActionName='<%# ListHelper.GetMainActionName((ContentRepository.Content)Container.DataItem) %>' IconVisible='false' >]]>
