...
<sn:DataSource ID="SNDSVersions" ContextInfoID="myContext" MemberName="Versions" FieldNames="Name DisplayName Version CheckInComments RejectReason" runat="server"  />
...
<div class="sn-pt-body-border ui-widget-content">
     <div class="sn-pt-body">
        ...
        <div class="sn-content">
            <div class="sn-floatleft"><%= Portal.UI.IconHelper.RenderIconTag(contextNode.Icon, null, 32)%></div>
        ...
        <tr class='sn-lg-row<%# Container.DisplayIndex % 2 %> ui-widget-content'>      		
          <td class="sn-lg-col-1">
              <sn:ActionLinkButton runat="server" ToolTip="View this version" ActionName="Browse" IncludeBackUrl="True"
                ContextInfoID="myContext" Text='<%# ((ContentRepository.Content)Container.DataItem).ContentHandler.Version.ToDisplayText() %>' ParameterString='<%# String.Concat("version=" + Eval("Version")) %>' />
          </td>
          ...
          <td class="sn-lg-col-3"><%# ContentRepository.Security.Sanitizer.Sanitize(Eval("CheckInComments") as string)%></td>
          <td class="sn-lg-col-4"><%# ContentRepository.Security.Sanitizer.Sanitize(Eval("RejectReason") as string)%></td>
