...
<%@ Import Namespace="ContentRepository.Storage.Data" %>
...
<% var allowedchildtypes = (PortalContext.Current.ContextNode as GenericContent).GetAllowedChildTypes().ToArray();
   var allowedfiletypes = allowedchildtypes.Where(ct => ct.IsInstaceOfOrDerivedFrom("File")).Select(ct => ContentRepository.Content.Create(ct)).OrderBy(ct => ct.DisplayName).ToArray();
   if (allowedchildtypes.Length == 0)
       allowedfiletypes = ContentRepository.Schema.ContentType.GetContentTypes().Where(ct => ct.IsInstaceOfOrDerivedFrom("File")).Select(ct => ContentRepository.Content.Create(ct)).OrderBy(ct => ct.DisplayName).ToArray();
   ...
   if (allowedfiletypes.Length > 0)
   {
       ...
       alltypesStr = allowedfiletypes.Length > 1 ? "<option value=\"Auto\">" + ResourceManager.Current.GetString("Action", "UploadAutoType") + "</option>" : "";
       alltypesStr += string.Join("", allowedtypesMarkup);
   }
%>

<% if (!ApplicationModel.UploadAction.AllowCreationForEmptyAllowedContentTypes(PortalContext.Current.ContextNode))
   { %>
<%= ResourceManager.Current.GetString("Action", "UploadExceptionEmptyAllowedChildTypes")%>
<% }
   else if (allowedfiletypes.Length == 0 && allowedchildtypes.Length != 0)
   { %>
<%= ResourceManager.Current.GetString("Action", "UploadNoAllowedTypeError")%>
<% }
   else
   { %>
<div class="sn-upload-area">
    <div class="sn-upload-buttonbar">
        <div id="sn-upload-fileuploadbutton" class="sn-submit sn-notdisabled sn-upload-button">
            <span><%= ResourceManager.Current.GetString("Action", "UploadAddFiles")%></span>
        ...
        <div class="sn-upload-type">
            <%= ResourceManager.Current.GetString("Action", "UploadUploadAs")%>
    ...
    <span id="sn-upload-othererror" style="display: none;"><%= ResourceManager.Current.GetString("Action", "UploadOtherError")%></span>
    <% } %>
    <div class="sn-upload-draganddrop">
        <%= ResourceManager.Current.GetString("Action", "UploadDragAndDrop")%>
    </div>
    <div class="sn-panel sn-buttons sn-upload-buttons">
        <div id="sn-upload-startbutton" class="sn-submit sn-notdisabled sn-upload-button sn-submit-disabled">
            <span><%= ResourceManager.Current.GetString("Action", "UploadStartUpload")%></span>
        </div>
        <input type="button" value='<%= ResourceManager.Current.GetString("Action", "Close")%>' class="sn-submit sn-closebutton">
    </div>
</div>
<script>
    var uploaddata = [];
    var maxChunkSize = <%= RepositoryConfiguration.BinaryChunkSize %>;
    var currentUser = '<%= ContentRepository.User.Current.Name %>';
    // 1 MB: 1048576
    //10 MB: 10485760
    //50 MB: 52428800
                    ...
                    if(inProgress){
                        if($('.overlay').length === 0){
                            overlayManager.showOverlay({
                                text: '<%= ResourceManager.Current.GetString("Controls", "AboortUploadFull")%>'
                            }); 
                        }
                        $popup = $('.overlay');
