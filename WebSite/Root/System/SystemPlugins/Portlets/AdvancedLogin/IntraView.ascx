...
<asp:LoginView ID="LoginViewControl" runat="server">
    ...
    <LoggedInTemplate>
      <div class="sn-login-leftpane">
        <span title="<%= ContentRepository.User.Current.Name%>"><%= String.Format(HttpContext.GetGlobalResourceObject("LoginPortlet","Welcome").ToString(), ContentRepository.User.Current.FullName) %></span>
      </div>
      <div class="sn-login-rightpane">
        <a class="sn-link" href='<%= Portal.PortletFramework.PortalActionLinkResolver.Instance.ResolveRelative(
                                                                            ContentRepository.User.Current.Path,
                                                                            "Edit",
                                                                            System.Web.HttpUtility.UrlEncode(Portal.Virtualization.PortalContext.Current.OriginalUri.ToString()))%>'>
