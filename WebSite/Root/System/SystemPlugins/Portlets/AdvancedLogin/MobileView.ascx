...
<asp:LoginView ID="LoginViewControl" runat="server">
         ...
         <asp:Login ID="LoginControl" runat="server" DisplayRememberMe="false" RememberMeSet="false" FailureText='<%$ Resources:LoginPortlet, FailureText %>' RenderOuterTable="false">
            <LayoutTemplate>
                <div class="snm-tile snm-tile-wide2 snm-tile-tall2">
                    <span class="snm-background"><img src="/Root/Skins/snmobile/images/logotile.png" width="176" height="176" alt="DMIS" /></span>
                </div>
    ...
    <LoggedInTemplate>
        <div id="snm-loggedin" class="snm-tile snm-tile-wide5">
            <div class="snm-floatleft snm-avatar"><img src="<%= Portal.UI.UITools.GetAvatarUrl(80, 80) %>" alt="" title="<%= ContentRepository.User.Current.FullName %>" /></div>
            <h2><%=GetGlobalResourceObject("LoginPortlet", "Welcome")%> <%= ContentRepository.User.Current.FullName %>!</h2>
            <span class="snm-fontsize3"><asp:LoginStatus ID="LoginStatusControl" LogoutText="<%$ Resources:LoginPortlet,Logout %>" LogoutPageUrl="/" LogoutAction="Redirect" runat="server" CssClass="snm-logout" /> &nbsp;/&nbsp; <a href="/"><%=GetGlobalResourceObject("LoginPortlet", "BackToHome")%></a></span>
        </div>
    </LoggedInTemplate>
</asp:LoginView>
        </div>
