...
<asp:LoginView ID="LoginViewControl" runat="server">
    ...
    <LoggedInTemplate>
        <%= HttpContext.GetGlobalResourceObject("LoginPortlet","LoggedIn") %> <strong><%= ContentRepository.User.Current.Name%></strong>
        <p style="margin: 1em 0;">
            <sn:ActionLinkButton ID="EditProfileLink" runat="server" ActionName="EditProfile" Text="<%$ Resources:LoginPortlet,EditProfile %>" ContextInfoID="myContext" />
            /
            <a class="snTextLink" href="/Book_Rental_Demo_Site/MyBooks"><%=GetGlobalResourceObject("LoginPortlet", "MyBooks")%></a>
            <br />
