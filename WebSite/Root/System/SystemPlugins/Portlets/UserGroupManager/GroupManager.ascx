...
<script runat="server">
    ...
    {
        var groupNode = ContentRepository.Content.Load(Eval("Path").ToString());
        return groupNode.Security.GetPermission(PermissionType.Save) == ContentRepository.Storage.Security.PermissionValue.Allow;
    }
    
    private string GetGroupNameText(ContentRepository.Group group)
    {
...
<asp:UpdatePanel ID="GroupManagerUpdatePanel" runat="server" UpdateMode="Conditional">
    <ContentTemplate>
        <asp:ListView ID="GroupList" runat="server" EnableViewState="false" DataMember="Path">
            ...
            <ItemTemplate>
                <asp:CheckBox runat="server" Text='<%#GetGroupNameText(Container.DataItem as ContentRepository.Group)%>'
        ...
        <table>
            <tr style="vertical-align:middle;">
                <td style="padding:8px 10px 8px 0;">
                    <asp:Button CssClass="sn-floatleft" runat="server" ID="btnUpdate" OnClick="UpdateMemberships" Text="<%$ Resources:GroupManager,UpdateMemberships %>" />
                </td>
                <td>
