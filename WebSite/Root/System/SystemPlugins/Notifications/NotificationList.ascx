...
<div class="sn-content-inlineview-header ui-helper-clearfix">
    <%= Portal.UI.IconHelper.RenderIconTag("user", null, 32)%>
...
<asp:Panel ID="pnlNotification" runat="server" CssClass="sn-notification" >
    <asp:ListView ID="ContentList" runat="server" EnableViewState="false">
        ...
        <ItemTemplate>
            <tr class='<%# (bool)((ContentRepository.Content)Container.DataItem)["IsActive"] ? "sn-active" : "sn-deactive" %>' runat="server">
                <td>
                    <%# HttpUtility.HtmlEncode((ContentRepository.Content.Load(Eval("ContentPath").ToString())).DisplayName) %>
                </td>
                ...
                <td>
                    <sn:ActionLinkButton runat="server" ID="BtnSetActivation" CssClass="sn-link" ActionName="SetActivation" ContextInfoID="ContextInfoUser" IconVisible="false" 
                        ...
                        <%# (bool)((ContentRepository.Content)Container.DataItem)["IsActive"]
    ...
    <sn:BackButton ID="BtnBack" runat="server" Text="<%$ Resources:Notifications,Back %>" CssClass="sn-submit" style="float: right;" />
</asp:Panel>