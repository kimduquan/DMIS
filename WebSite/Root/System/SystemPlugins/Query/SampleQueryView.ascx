<%@ Language="C#" AutoEventWireup="true" Inherits="Portal.UI.Controls.QueryView" %>
<%@ Register Src="/Root/System/SystemPlugins/Query/PagerControl.ascx" TagName="PagerControl" TagPrefix="sn" %>
<%-- 
Query example:

<?xml version="1.0" encoding="utf-8"?>
<SearchExpression xmlns="http://schemas.com/ContentRepository/SearchExpression">
  <And>
		...
		<sn:RepeaterView ID="RepeaterView1" runat="server">
			<ItemTemplate>
				...
				<div class="sn-content sn-contentlist-item">
						    ...
						    <h1 class="sn-content-title"><a href="/features/News/Article?Content=<%# GetValue(Container.DataItem, "Path") %>"><%# HttpUtility.HtmlEncode(GetValue(Container.DataItem, "DisplayName"))%></a></h1>
					    ...
					    <div class="sn-more">
						    <a class="sn-link" href="/features/News/Article?Content=<%# GetValue(Container.DataItem, "Path") %>"><%=GetGlobalResourceObject("PagerControl", "more")%> &raquo;</a>
					    </div>
