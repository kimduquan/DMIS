...
<%@ Control Language="C#" AutoEventWireup="false" Inherits="Portal.UI.Controls.PermissionEditor" %>
...
<script runat="server">
    string displayName = ContentRepository.Content.Create(PortalContext.Current.ContextNode).DisplayName;

    int visiblePermissionCount = 18;
    
    protected override void OnInit(EventArgs e)
    {
        ((Portal.UI.Controls.PermissionEditor)this).VisiblePermissionCount = visiblePermissionCount;
        base.OnInit(e);
    }
</script>

  <div style="float:left"><%= Portal.UI.IconHelper.RenderIconTag("security", null, 32) %></div>
         ...
         <h2 class="sn-permissions-title" style="clear:both">
            ...
            <div style="float:right;">            
                <sn:ActionLinkButton ID="ActionLinkButtonEditMembers" runat="server" ActionName="Edit" 
                    ...
                    Visible='<%# Node.LoadNode((Container.DataItem as SnAccessControlEntry).Identity.Path).NodeType.IsInstaceOfOrDerivedFrom("Group") && !RepositoryConfiguration.SpecialGroupPaths.Contains((Container.DataItem as SnAccessControlEntry).Identity.Path) %>' />
