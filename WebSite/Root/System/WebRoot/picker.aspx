...
<%
    ...
    if (!string.IsNullOrEmpty(allowedContentTypes))
    {
        ...
        contentTypes = allowedContentTypesArray.Select(c => ContentRepository.Schema.ContentType.GetByName(c)).Where(ct => ct != null).ToList();
    }
    else
    {
        if (!string.IsNullOrEmpty(targetPath))
        {
            var targetNode = Node.LoadNode(targetPath) as ContentRepository.GenericContent;
            if (targetNode != null)
            {
                ...
                else
                {
                    ...
                    var targetContent = ContentRepository.Content.Create(targetNode);
                    bool found;
    ...
    var allTypes = ContentRepository.Schema.ContentType.GetContentTypes();
    ...
    <div id="sn-contentpicker-contenttypeslist" class="ui-widget-content ui-corner-all">
        <% 
           foreach (var contentType in contentTypes)
           {
               ...
               var title = HttpUtility.HtmlEncode(ContentRepository.Content.Create(contentType).DisplayName);
