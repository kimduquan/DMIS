...
<xsl:stylesheet
    ...
    xmlns:snf="sn://Portal.UI.XmlFormatTools"
    xmlns:acf="sn://ApplicationModel.ActionFramework"
  ...
  <xsl:template match="/">
    ...
    <xsl:if test="$uploadLink!=''">
      <xsl:variable name="newimage" select="snc:GetResourceString('$Renderers, NewImage')" />
      <a href="{Content/SelfLink}?action=upload&amp;back={Content/SelfLink}" style="background-image:url(/Root/Global/images/icons/16/newimage.png)" class="sn-actionlinkbutton icon">
        <xsl:value-of select="$newimage"/>
      </a>
    </xsl:if>
