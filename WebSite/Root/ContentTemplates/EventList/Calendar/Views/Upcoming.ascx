<%@ Control Language="C#" AutoEventWireup="false" Inherits="Portal.UI.ContentListViews.UpcomingEventView" %>
    ...
    <HeaderTemplate>
        <div class="snEventList">
            <div class="inner">
                ...
                <h2 class="snEventListTitle"><%=GetGlobalResourceObject("EventCalendarPortlet", "EventsOn")%><asp:Label ID="DateLabel" runat="server" /> :</h2>

    </HeaderTemplate>
    <ItemTemplate>  
                <div class="snEvent">
                    <a href='<%# Portal.Virtualization.PortalContext.Current.GetContentUrl(Container.DataItem) %>'><%# Eval("DisplayName") %></a><br />
                    <small><%=GetGlobalResourceObject("EventCalendarPortlet", "StartDate")%><strong><%# Eval("StartDate") %></strong></small>
                </div>
    </ItemTemplate>
    <FooterTemplate>
            </div>
