...
<%
    var userProfile = this.Content.ContentHandler as ContentRepository.UserProfile;
    var email = userProfile.User.Email;
    var phone = userProfile.User["Phone"] as string;
    var managerRefList = this.Content["Manager"] as IEnumerable<ContentRepository.Storage.Node>;
    var manager = managerRefList == null ? null : managerRefList.FirstOrDefault() as ContentRepository.User;
    ...
    var username = ContentRepository.User.Current.Name;
    var profileUsername = userProfile.User.Username;

    bool showEducationAndWork = !string.IsNullOrEmpty(department) && !string.IsNullOrEmpty(languages) && !Portal.Helpers.UserProfiles.IsEducationEmpty(education);
        ...
        <div id="snm-userprofile-centercol">
          <div id="snm-userprofile-leftcol">
            <a href='<%= userProfile.User.AvatarUrl %>' title='<%= userProfile.User.FullName %>' rel="sexylightbox">
              <img src='<%= Portal.UI.UITools.GetAvatarUrl(userProfile.User, 150, 150) %>' alt='<%=GetGlobalResourceObject("Content", "MissingImage")%>' class="sn-avatar nice_img" />
            </a>
          ...
          <div id="snm-userprofile-content">
              <div id="snm-userprofile-info" class="snm-tab-active">
                  ...
                  <dl class="snm-property-list">                    
                      ...
                      <% } %>
                      <% if (!string.IsNullOrEmpty(phone)) { %><dt>phone</dt><dd><%= ContentRepository.i18n.ResourceManager.Current.GetString("UserBrowse", "PhoneLabel") %>: <%= phone %></dd>  <% } %>
                      <% if (manager != null) { %>
                          <dt><%= ContentRepository.i18n.ResourceManager.Current.GetString("UserBrowse", "ManagerLabel")%>:</dt>
                  ...
                  <% if(showEducationAndWork){ %>
                      ...
                      <dl class="snm-property-list">
                          <% if (!string.IsNullOrEmpty(department)) { %>
                              <dt><%= ContentRepository.i18n.ResourceManager.Current.GetString("UserBrowse", "DepartmentLabel") %>:</dt><dd><%= department%></dd>
                          <% } %>
                          <% if (!string.IsNullOrEmpty(languages)) { %>
                              <dt><%= ContentRepository.i18n.ResourceManager.Current.GetString("UserBrowse", "LanguagesLabel")%>:</dt><dd><%= languages%></dd>
                          <% } %>
                          <% if (!Portal.Helpers.UserProfiles.IsEducationEmpty(education)) { %>
