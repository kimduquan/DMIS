...
<%var wsContent = ContentRepository.Content.Load(this.Content.WorkspacePath); %>

<div class="sn-contentlist sn-blogpost-browse">
    <div class="sn-blogpost ui-helper-clearfix">
        <% 
            var unpublished = Content["IsPublished"].Equals(false) || (DateTime)Content["PublishedOn"] >= DateTime.UtcNow;
            if (unpublished && !Security.IsInRole("Editors"))
            {%>
                <div class="sn-blogpost-unpublished">                    
                    <span><%= ResourceManager.Current.GetString("Portal", "SnBlog_BlogPostsPortlet_UnpublishedPost")%></span>
                </div>
            <%}
            else
            {
                ...
                if (showAvatar)
                { %>
                <div style="float: left; margin-right: 10px;"><img style="margin-top: 5px" src='<%= UITools.GetAvatarUrl(this.Content["CreatedBy"] as ContentRepository.User, 48, 48) %>' class='sn-entry-avatar' alt='<%# Eval("CreatedBy") %>' title='' /></div>
                <%} %>  
                <div>
                    <div class="sn-blogpost-admin">
                        <% var editUrl = Actions.ActionUrl(this.Content, "Edit", true);
                           ...
                           {%>
                                <div class="sn-blogpost-admin-item"><img src="/Root/Global/images/icons/16/edit.png" alt="<%=GetGlobalResourceObject("Content", "Edit")%>" /><a href="<%= editUrl %>"><%= ResourceManager.Current.GetString("Portal", "SnBlog_BlogPostsPortlet_EditPost")%></a></div>
                            <%}
                    ...
                    <div class="sn-blogpost-info">
                        ...
                        <span class="sn-blogpost-createdby"><%= (this.Content["CreatedBy"] as ContentRepository.User).FullName%></span> - 
                        ...
                        <% var i = 0;
                           ...
                           { 
                            i++;
                            %><%= i > 1 ? ", " : " - " %><a class="sn-blogpost-tag" href="<%= Actions.ActionUrl(wsContent, "Search", false) + "&text=" + HttpUtility.UrlEncode(tag) %>"><%= HttpUtility.UrlEncode(tag)%></a><%} %>
                     </div>
                </div>
