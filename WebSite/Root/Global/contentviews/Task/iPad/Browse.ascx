...
<script runat="server">
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        foreach (var opt in (this.Content.Fields["Status"].FieldSetting as ContentRepository.Fields.ChoiceFieldSetting).Options)
        {
        ...
        foreach (var opt in (this.Content.Fields["Priority"].FieldSetting as ContentRepository.Fields.ChoiceFieldSetting).Options)
        {
...
<article class="snm-tile bg-zero" id="reload">
  <a href="javascript:location.reload(true)" class="snm-link-tile bg-zero clr-text">
    <span class="snm-lowertext snm-fontsize3"><asp:Literal ID="Literal1" runat="server" Text="<%$ Resources:Content,Refresh%>" /></span>
  </a>
</article>
<article class="snm-tile" id="backtile">
  <a href="javascript:window.history.back()" class="snm-link-tile bg-semitransparent clr-text">
    <span class="snm-lowertext snm-fontsize3"><asp:Literal ID="Literal2" runat="server" Text="<%$ Resources:Content,Back%>" /></span>
  </a>
</article>
    ...
    <div class="snm-pagecontent">
      <div class="snm-col">
        ...
        <dl class="snm-property-list">          
          <dt><%=GetGlobalResourceObject("Content", "StartDate")%></dt><dd><%= DateTime.Parse(this.Content["StartDate"].ToString()) == DateTime.MinValue ? "n/a" : DateTime.Parse(this.Content["StartDate"].ToString()).ToString(System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat.RFC1123Pattern)%></dd>
          <dt><%=GetGlobalResourceObject("Content", "DueDate")%></dt><dd><%= DateTime.Parse(this.Content["DueDate"].ToString()).ToString(System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat.RFC1123Pattern) %></dd>                    
          <dt><%=GetGlobalResourceObject("Content", "Description")%></dt><dd><%= this.Content["Description"] %></dd>
          <dt><%=GetGlobalResourceObject("Content", "AssignedTo")%></dt>
          <dd>
            <% 
                var users = this.Content.Fields["AssignedTo"].GetData() as List<ContentRepository.Storage.Node>;
                if (users != null && users.Count > 0)
                {
                    foreach (var user in users)
                    {
                        var tmpUser = user as ContentRepository.User;
                        if (tmpUser != null)
                        {
                            var avatarUrl = Portal.UI.UITools.GetAvatarUrl(tmpUser, 32, 32);
                            var fullName = String.IsNullOrEmpty(tmpUser.FullName) ? tmpUser.DisplayName : tmpUser.FullName;                                                      
                            %><a class="snm-avatar-user clickable-yellowbg" style="background-image: url(<%= avatarUrl %>);" href="<%= Portal.Helpers.Actions.ActionUrl(ContentRepository.Content.Create(tmpUser), "Profile", true) %>" ><%= fullName %></a><%
                        }
                    }
                }
                else
          ...
          <dt><%=GetGlobalResourceObject("Content", "Completion")%></dt><dd><%= this.Content["TaskCompletion"] %>%</dd>
          <dt></dt><dd></dd>
        </dl>
        <h2><%=GetGlobalResourceObject("Content", "Actions")%></h2>
        <dl class="snm-property-list">
          <dt><%=GetGlobalResourceObject("Content", "Status")%></dt>
          <dd>
          ...
          <dt><%=GetGlobalResourceObject("Content", "Priority")%></dt>
          <dd>
