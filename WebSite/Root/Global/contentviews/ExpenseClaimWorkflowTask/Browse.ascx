...
<div class="sn-inputunit ui-helper-clearfix" id="InputUnitPanel1">
    ...
    <div class="sn-iu-control">
        ...
        <a href='<%= Actions.BrowseUrl(ContentRepository.Content.Create(eClaim)) %>' title='<%= HttpUtility.HtmlEncode(eClaim.DisplayName) %>'><%=HttpUtility.HtmlEncode(eClaim.DisplayName) %></a> (in 
        <a href='<%= Actions.BrowseUrl(ContentRepository.Content.Create(eClaim.Parent)) %>' title='<%=eClaim.ParentPath %>' target="_blank"><%=HttpUtility.HtmlEncode(eClaim.Parent.DisplayName)%></a>)     
    </div>
</div>
   <% } %>
...
<% var status = this.Content.ContentHandler.GetProperty<string>("Result");
   ...
   { 
       ...
       if (gc != null && SavingAction.HasApprove(gc) && this.Content.Security.HasPermission(ContentRepository.Storage.Schema.PermissionType.Save))
       {%>
