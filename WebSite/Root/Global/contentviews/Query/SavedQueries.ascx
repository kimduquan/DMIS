...
<input type="hidden" class="currentusername" value='<%= ((ContentRepository.User)ContentRepository.User.Current).Name %>' />
<input type="hidden" class="currentworkspaceparent" value='<%= Portal.Virtualization.PortalContext.Current.Workspace.ParentPath %>' />
<input type="hidden" class="currentworkspace" value='<%= Portal.Virtualization.PortalContext.Current.Workspace.Name %>' />
<script>
    ...
    //      var getPublicSavedQueries = odata.customAction({
    //          path: workspace,
    //          action: "GetQueries",
    //          params: { onlyPublic: true },
    //          $select: ['Query', 'Path', 'DisplayName', 'Actions'],
    //          $expand: 'Actions',
    //          $orderby: "DisplayName",
    //          metadata: 'no'
    //      }).done(function (data) {
    //          savedQueries.public = data.d.results;
    //      });
    ...
    $.when(getSavedQueries).done(function () {
        ...
        var func = function (i, query) {
            ...
            if (hasDeletePermission) {
                savedQuery = '<div class="savedQuery" data-querypath="' + this.Path + '" data-querytype="' + queryType + '"><span class="title" title="' + SN.Util.Sanitize(query.DisplayName) + '">' + SN.Util.Sanitize(query.DisplayName) + '</span><span class="data-query hidden">' + query.Query + '</span><span class="saved-search-delete" title="' + SN.Resources.QueryBuilder["Delete"] + '"></span></div>'
            }
            else {
