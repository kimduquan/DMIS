...
<div class="sn-content sn-workflow sn-content-inlineview">
        ...
        <h2 class="sn-content-title"><%= Portal.UI.IconHelper.RenderIconTag(this.Icon, null, 32) %><%=GetGlobalResourceObject("Workflow", "Start")%> <strong><%= (this.Content.ContentHandler as GenericContent).DisplayName%></strong> <%=GetGlobalResourceObject("Workflow", "On")%> <strong><%= ((Node)this.Content["RelatedContent"]).DisplayName %></strong><%=GetGlobalResourceObject("Workflow", "N")%></h2> 
