...
<script runat="server">
    ...
    protected void OnSetPassword(object sender, EventArgs e)
    {
        ...
        using (new SystemAccount())
        {
            ...
            {
                user.Password = p1;
                user.PasswordHash = ContentRepository.Fields.PasswordField.EncodePassword(p1, user);
                user.Save();
    ...
    protected User GetUser()
    {
        using (new SystemAccount())
        {
            ...
            return ContentRepository.Content.All.OfType<User>().Where(u => u.Email == email).FirstOrDefault();
        }
    }
