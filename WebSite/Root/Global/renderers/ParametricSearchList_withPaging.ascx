...
public string GetNonSpecificParams()
{
    ...
    foreach (var key in HttpContext.Current.Request.Params.AllKeys)
    {
        ...
        if (!key.Contains(Math.Abs((PortalContext.Current.ContextNodePath + this.Model.State.Portlet.ID).GetHashCode()).ToString()) && HttpContext.Current.Request.Url.ToString().Contains(key))
        {
