...
<%@ Import Namespace="ContentRepository.Schema" %>
...
<%
    var settings = new Search.QuerySettings { EnableAutofilters = Search.FilterStatus.Disabled };
    var workspaceGroups = Search.ContentQuery.Query("+TypeIs:Group +InTree:\"" + PortalContext.Current.ContextWorkspace.Path + "\"", settings).Nodes.ToList();
%>
...
<%
    var members = new[] { new { Group = null as ContentRepository.Group, Member = null as Node } }.ToList();
    members.Clear();
    foreach (var groupNode in workspaceGroups)
    {
        var group = groupNode as ContentRepository.Group;
        if (group == null)
            continue;
...
<div class="sn-workspacemembers">
    <% var editable = (this.Parent as ContextBoundPortlet).ContextNode.Security.HasPermission(ContentRepository.Storage.Schema.PermissionType.Save); %>
