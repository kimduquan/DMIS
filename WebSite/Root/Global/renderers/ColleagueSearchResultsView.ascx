    ...
    <ItemTemplate>
        <div class="sn-user">           
            <sn:ActionLinkButton NodePath='<%#Eval("Path") %>' ActionName="Browse" IconVisible="false"
                ...
                Height="64" Width="64" AlternateText="<%$ Resources:Renderers,MissingUserImage %>" BorderColor="White" BorderStyle="Solid" BorderWidth="1px" />
                
                <div class="sn-user-properties">
                    <table>
                        <tr><td><%#Eval("FullName") %> (<%#Eval("Name") %>)</td></tr>
                        <tr><td><%=GetGlobalResourceObject("Renderers", "Department")%><td></td> <%#Eval("Department") %></td></tr>
                        <tr><td><%=GetGlobalResourceObject("Renderers", "Email")%> <td></td><%#Eval("Email") %></td></tr>
                    </table>
                </div>             
            </sn:ActionLinkButton>
        </div>
