...
<% var file = this.Content.ContentHandler as ContentRepository.File;
   var filePath = file.Path;
   var version = file.Version.ToString();
   var ext = ContentRepository.ContentNamingHelper.GetFileExtension(file.Name);
   ...
   var previews = ContentRepository.Storage.RepositoryPath.Combine(file.Path, "Previews");
   ...
   var createdby = (file.CreatedBy as ContentRepository.User).FullName;
   var modifiedby = (file.ModifiedBy as ContentRepository.User).FullName;
   var lockedby = file.Locked ? (file.LockedBy as ContentRepository.User).FullName : string.Empty;
   var createdbyc = ContentRepository.Content.Create(file.CreatedBy);
   var modifiedbyc = ContentRepository.Content.Create(file.ModifiedBy);
   var lockedbyc = file.Locked ? ContentRepository.Content.Create(file.LockedBy as ContentRepository.User) : null;

   // These are needed on the client side; eliminating needless AJAX requests
   var canSave = PortalContext.Current.ContextNode.Security.HasPermission(ContentRepository.User.Current, PermissionType.Save);
   var canPreviewWithoutRedaction = PortalContext.Current.ContextNode.Security.HasPermission(ContentRepository.User.Current, PermissionType.PreviewWithoutRedaction);
   var canPreviewWithoutWatermark = PortalContext.Current.ContextNode.Security.HasPermission(ContentRepository.User.Current, PermissionType.PreviewWithoutWatermark);

   var resourceScript = Portal.Resources.ResourceScripter.RenderResourceScript("DocViewer", System.Globalization.CultureInfo.CurrentUICulture);

   var enterpriseEdition = RepositoryVersionInfo.Instance.OfficialVersion.Edition == "Community";
%>
...
<input type="hidden" class="currentcontent" value='<%= Portal.Virtualization.PortalContext.Current.ContextNodePath %>' />
<input type="hidden" class="currentparent" value='<%= Portal.Virtualization.PortalContext.Current.ContextNode.ParentPath %>' />
<input type="hidden" class="currentnode" value='<%= Portal.Virtualization.PortalContext.Current.ContextNode.Name %>' />
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        var mdate = '<%= file.ModificationDate.ToString(System.Globalization.CultureInfo.GetCultureInfo("en-US")) %>';
        var cdate = '<%= file.CreationDate.ToString(System.Globalization.CultureInfo.GetCultureInfo("en-US")) %>';

        modDate = SN.Util.setFriendlyLocalDateFromValue(mdate, '<%= System.Globalization.CultureInfo.CurrentUICulture%>');
        creDate = SN.Util.setFriendlyLocalDateFromValue(cdate, '<%= System.Globalization.CultureInfo.CurrentUICulture%>');
        ...
        function printDocument() {
            ...
            var pics = '<style type="text/css">img{display:block;max-width:100% !important;}</style>';
...
<% if (RepositoryVersionInfo.Instance.OfficialVersion.Edition != "Community")
   { %>
<style>
    .enterpriseLicence {
        position: absolute;
