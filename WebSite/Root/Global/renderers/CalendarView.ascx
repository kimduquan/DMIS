...
<script runat="server">
    ...
    private IEnumerable<ContentRepository.Content> GetEventsForDay(DateTime matchDate)
    {
        //we need to use AsEnumerable here because 'let + select' does not work in LINQtoSN
        return (from content in this.Model.Items.AsEnumerable()
    ...
    private IEnumerable<ContentRepository.Content> GetEventsForMonth(DateTime matchDate)
    {
        //we need to use AsEnumerable here because 'let + select' does not work in LINQtoSN
        return (from content in this.Model.Items.AsEnumerable()
                let mds = Convert.ToDateTime(content["StartDate"])
    ...
    <script>
        $(function ()
        {
            $(".sn-cal-day>a[disabled='disabled']").each(function ()
            {
                    $(this).remove();
            });
        });
        ...
        <asp:Repeater ID="DetailsControl" runat="server" >
            <ItemTemplate>  
                ...
                <div class="sn-event ui-helper-clearfix">
                    ...
                    <sn:ActionMenu NodePath='<%# Eval("Path") %>' runat="server" WrapperCssClass="sn-floatright" Scenario="ListItem" Text="<%$ Resources:Renderers,ManageEvent %>" />
                    <% } %>
