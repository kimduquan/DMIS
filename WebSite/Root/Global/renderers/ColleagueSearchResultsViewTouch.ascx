    ...
    <ItemTemplate>
        <div id="sn-tch-user" class="sn-tch-user">
            <sn:ActionLinkButton ID="ActionLinkButton1" NodePath='<%#Eval("Path") %>' ActionName="Browse"
                ...
                Height="128" Width="128" AlternateText="<%$ Resources:Renderers,MissingUserImage %>" BorderColor="White" BorderStyle="Solid" BorderWidth="1px" />
                </div>
                            
                <div class="sn-user-properties">
                    <div class="sn-tch-prop"><h1><%#Eval("FullName") %> (<%#Eval("Name") %>)</h1></div>
                    <div class="sn-tch-prop"><h2><%=GetGlobalResourceObject("Renderers", "Department")%> <%#Eval("Department") %></h2></div>
                    <div class="sn-tch-prop"><h2><%=GetGlobalResourceObject("Renderers", "Email")%> <%# Eval("Email") %></h2></div>
                </div>           
            </sn:ActionLinkButton>
        </div>
    </ItemTemplate>
