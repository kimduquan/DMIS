...
<sn:DataSource ID="SNDSFields" ContextInfoID="ViewContext" MemberName="AllFieldSettingContents" FieldNames="Name DisplayName ShortName Owner Path" runat="server"  />
...
<asp:ListView ID="FieldsContentList" DataSourceID="SNDSFields" runat="server" EnableViewState="false" >
       ...
       <ItemTemplate>
            <tr class="sn-lg-row0 ui-widget-content">
                ...
                <td><%# Eval("Name") %></td>
                <td><sn:ActionLinkButton runat="server" ID="ActionLink" ActionName="Explore" NodePath='<%# ((ContentRepository.Storage.Node)((ContentRepository.Content)Container.DataItem)["Owner"]).Path %>' Text='<%# Eval("Owner")%>' IconVisible="false" /></td>
                <td><%# HttpUtility.HtmlEncode(Eval("ShortName"))%></td>
                <td><%# ((ContentRepository.Content)Container.DataItem)["Description"]%></td>
            </tr>
       </ItemTemplate>
       <EmptyDataTemplate>
       </EmptyDataTemplate>
