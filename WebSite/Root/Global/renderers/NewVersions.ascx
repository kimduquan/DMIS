...
<script>
    ...
    {
        $.getScript("http://www.com/Root/Sites/Default_Site/version.js?callback=?", function ()
        {

            var name = (Version.name);
            var versionnumber = (Version.versionnumber);
            var version = (Version.versionnumber).replace(/\D/g, '');
            var edition = (Version.edition);
            var releasedate = (Version.releasedate);
            var changeloglink = (Version.changeloglink);
            var downloadlink = (Version.downloadlink);
            ...
            else if (currvernum == version && status != "stable")
            {
                ...
                $('.info').html('This is the latest version of DMIS').css('display', 'block');
