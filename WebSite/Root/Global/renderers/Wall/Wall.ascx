...
<div>
    ...
    <% 
        ...
        var postAllowed = Portal.Wall.WallHelper.HasWallPermission(wsPath, null);
    %>
    <% if (postAllowed) { %>
    ...
    <% } %>
    <% var pageSize = (this.Parent as Portal.Portlets.Wall.WallPortlet).PageSize; %>
    <input type="hidden" class="sn-posts-workspace-path" value=<%= Portal.Virtualization.PortalContext.Current.ContextWorkspace.Path %> />
