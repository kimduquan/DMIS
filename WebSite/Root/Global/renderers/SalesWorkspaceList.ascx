    ...
    <sn:Toolbar runat="server">
        <sn:ToolbarItemGroup Align="left" runat="server">
            <sn:ActionMenu ID="ActionMenu1" runat="server" Scenario="New" ContextInfoID="myContext" RequiredPermissions="AddNew" CheckActionCount="True">
                <sn:ActionLinkButton ID="ActionLinkButton1" runat="server" ActionName="Add" IconUrl="/Root/Global/images/icons/16/newfile.png" ContextInfoID="newWS" Text='<%$ Resources: Scenario, New %>' CheckActionCount="True" NodePath="/Root/Sites/Default_Site/workspaces/Sales" ParameterString="ContentTypeName=/Root/ContentTemplates/SalesWorkspace/Sales_Workspace"/>
            </sn:ActionMenu>     
        </sn:ToolbarItemGroup>
    </sn:Toolbar>
        ...
        <div class="sn-ws-listitem sn-ws-sales ui-widget ui-widget-content ui-corner-all ui-helper-clearfix">
            ...
            <div class="sn-layoutgrid ui-helper-clearfix">
                ...
                <div class="sn-layoutgrid-column sn-layoutgrid2">
                    ...
                    <strong><%=GetGlobalResourceObject("KPIRenderers", "ExpectedRevenue")%></strong><div class="sn-progress sn-kpi-revenue"><span style="width:<%= expectedRevenue.HasValue ? (expectedRevenue.Value/maxRevenue.Value*100).ToString() : "0"%>%"><%= expectedRevenue.HasValue ? expectedRevenue.Value.ToString() : "?"%>&curren;</span></div>
                    <strong><%=GetGlobalResourceObject("KPIRenderers", "ChanceToWin")%></strong><div class="sn-progress sn-kpi-chance"><span style="width:<%= chanceToWin.HasValue ? chanceToWin.Value.ToString() : "0"%>%"><%= chanceToWin.HasValue ? chanceToWin.Value.ToString() : "0"%>%</span></div>
                    <strong><%=GetGlobalResourceObject("KPIRenderers", "Progress")%></strong><div class="sn-progress sn-kpi-progress"><span style="width:<%= progress.HasValue ? progress.Value.ToString() : "0"%>%"><%= progress.HasValue ? progress.Value.ToString() : "0"%>%</span></div>
                </div>
            </div>
        </div>
