...
<script type="text/javascript" language="javascript">
    $(function () {
        if (<%# ((SNControls.DatePicker)Container).Mode != SNFields.DateTimeMode.None ? "true" : "false" %>) {
            ...
            var fieldNameClass = '<%# UITools.GetFieldNameClass(Container) %>';
            ...
            $timeZoneOffset = $('input.sn-ctrl-timezoneoffset-' + fieldNameClass);

            var today = new Date(new Date(SN.Util.mergeDateAndTimeFieldLocal(originalDate, $dateField.next('input.sn-ctrl-time-' + fieldNameClass).val())));
            $dateField.siblings('input.sn-ctrl-timezoneoffset-' + fieldNameClass).val(today.getTimezoneOffset());

            $dateField.on('change', function(){
                SN.Util.onDatePickerSelect(fieldNameClass);
            });
