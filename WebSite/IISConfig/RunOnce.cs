...
namespace Portal.Setup
{
    internal class RunOnce
    {
        ...
        private static void SaveInitialIndexDocuments()
        {
            ...
            var idSet = ContentRepository.Storage.Data.DataProvider.LoadIdsOfNodesThatDoNotHaveIndexDocument();
        ...
        private static void TreeWalker(string path, bool pathIsFile, Node folder, string indent, List<ContentInfo> postponedList)
        {
            ...
            foreach (ContentInfo contentInfo in contentInfos)
            {
                ...
                try
                {
                    ...
                    // turn off some observers
                    content.ContentHandler.DisableObserver(typeof(Preview.DocumentPreviewObserver));
        ...
        private static void SetPermissions()
        {
            try
            {
                var devs = Node.Load<OrganizationalUnit>("/Root/IMS/BuiltIn/Demo/Developers");
        ...
        private static void CreateLog(string logPath)
        {
            ...
            wr.WriteLine("Start importing ", DateTime.UtcNow, Environment.NewLine, "Log file: ", _logFilePath);
            wr.WriteLine();
            wr.Close();
            _lineStart = true;
        }
