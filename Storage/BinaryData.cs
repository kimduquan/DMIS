...
namespace ContentRepository.Storage
{
	...
	public class BinaryData : IDynamicDataAccessor
	{
		...
		private static Stream CloneStream(Stream stream)
		{
            ...
            var snFileStream = stream as ContentRepository.Storage.Data.SqlFileStream;
            if (snFileStream != null)
                return new ContentRepository.Storage.Data.SqlFileStream(snFileStream.Length, snFileStream.BinaryPropertyId);
        ...
        private static void AssertChunk(int contentId, string fieldName, out Node node, out PropertyType propertyType)
        {
            ...
            if (node.LockedById != -1 && node.LockedById != currentUser.Id)
                throw new SecurityException(contentId, "It is only allowed to upload a binary chunk if the content is locked by the current user.");
