...
namespace ContentRepository.Storage.Schema
{
	internal sealed class NodeTypeManager : SchemaRoot
    {
        ...
        [Serializable]
        internal class NodeTypeManagerRestartDistributedAction : Communication.Messaging.DistributedAction
        {
        ...
        internal static void Restart()
        {
            Logger.WriteInformation(EventId.TypeSystem.NodeTypeManagerRestart,
                "NodeTypeManager.Restart called.", 
		...
		private void StartEventSystem()
		{
            ...
            Logger.WriteInformation(EventId.SystemStart.NodeObserversInstantiated, "NodeObservers are instantiated. ",
            properties: new Dictionary<string, object> { { "Types", String.Join(", ", _nodeObservers.Select(x => x.GetType().FullName).ToArray()) } });
		}
