...
namespace ContentRepository.Storage.Schema
{
	...
	{
        public static readonly string RepositoryStorageSchemaXmlNamespace = "http://schemas.com/ContentRepository/Storage/Schema";
        ...
        private static long GetSchemaTimestamp(DataSet dataSet)
        {
            ...
            if (rows.Count == 0)
                return 0;
            var row = rows[0];
            return ContentRepository.Storage.Data.SqlClient.SqlProvider.GetLongFromBytes((byte[])row["Timestamp"]);
        }
		...
		private void BuildPermissions(DataTable table)
		{
			foreach (DataRow row in table.Rows)
			{
                ...
                int index = TypeConverter.ToInt32(row["PermissionIndex"]);
                string name = TypeConverter.ToString(row["Name"]);
                CreatePermissionType(id, index, name);
			}
		}
		...
		{
            sb.Append(indent).AppendLine(String.Format(CultureInfo.CurrentCulture, "<PermissionType itemID=\"{0}\" name=\"{1}\" itemIndex=\"{2}\" />", permType.Id, permType.Name, permType.Index)); //@@ PermissionType.Id
		}
		...
		private static DataSet CreateDataSet(Dictionary<string, int> dataTypes, Dictionary<PropertySetType, int> propertySetTypes)
		{
            ...
            table.Columns.Add("PermissionIndex", typeof(int));
            table.Columns.Add("Name", typeof(string));
		...
		private static void BuildPermissionRow(XmlNode node, DataTable table)
		{
            ...
            int id = GetIntFromXmlAttribute(node, "itemID");
            int index = GetIntFromXmlAttribute(node, "itemIndex");
            string name = GetStringFromXmlAttribute(node, "name");

			table.Rows.Add(id, index, name);
		}
		...
		{
			return CreatePermissionType(0, 0, name);
		}
        private PermissionType CreatePermissionType(int id, int index, string name)
        {
            PermissionType perm = new PermissionType(id, index, name, this);
