...
namespace ContentRepository.Storage
{
    ...
    /// <summary>
    /// <para>Represents a structured set of data that can be stored in the Content Repository.</para>
    /// <para>A node can be loaded from the Content Repository, the represented data can be modified via the properties of the node, and the changes can be persisted back.</para>
    /// </summary>
    ...
    public abstract class Node : IPasswordSaltProvider
    {
        ...
        /// <summary>
        /// Gets the <see cref="ContentRepository.Storage.Schema.NodeType">NodeType</see> of the instance.
        /// </summary>
        ...
        /// </summary>
        /// <remarks>
        /// A node is uniquely identified by the Name within a leaf in the Content Repository tree.
        ...
        public virtual Node CreatedBy
        {
            ...
            set
            {
                ...
                if (value.Id == RepositoryConfiguration.SomebodyUserId)
                    throw new SecurityException("Cannot set the CreatedBy property with the Somebody user");
                if (!(value is IUser))
        ...
        public virtual Node ModifiedBy
        {
            ...
            set
            {
                ...
                if (value.Id == RepositoryConfiguration.SomebodyUserId)
                    throw new SecurityException("Cannot set the ModifiedBy property with the Somebody user");
                if (!(value is IUser))
        ...
        /// </summary>
        /// <example>How to load a Node by passing the Content Repository path.
        ...
        /// </summary>
        /// <example>How to load the version 2.0 of a Node by passing the Content Repository path.
        /// In this case you will get a Node named node filled with the data of the latest version of Node /Root/MyFavoriteNode.
        /// <code>
        ...
        private static AccessLevel GetUserAccessLevel(NodeHead head)
        {
            ...
            switch (SecurityHandler.GetPermittedLevel(head))
            {
                case PermittedLevel.None:
                    throw new SecurityException(head.Path, PermissionType.See, AccessProvider.Current.GetCurrentUser(), "Access denied.");
                ...
                if (settings.LockerUserId != 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!settings.TakingLockOver && this.LockedById != AccessProvider.Current.GetCurrentUser().Id)
                            throw new SecurityException(this.Id, "Node is locked by another user");
                        LastLockUpdate = DateTime.UtcNow;
                    }
                }
                else
