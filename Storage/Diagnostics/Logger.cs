...
namespace Diagnostics
{
    public class Logger
    {
        #region Deletable deprecated items
        [Obsolete("Use inline array initializers instead", true)]
        public static IEnumerable<string> Categories(params string[] categories)
        {
            return categories;
        }
        [Obsolete("Use Logger.EventId.NotDefined instead", true)]
        public static int DefaultEventId = 1;
        [Obsolete("Use null instead", true)]
        ...
        {
            Logger.Current.LoggerAdapter.Write(auditEvent, new[] { "Audit" }, DefaultPriority, auditEvent.EventId,
