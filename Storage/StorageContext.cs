        ...
        public static void RemoveUnnecessaryDirectories()
        {
            ...
            foreach (var dir in unnecessaryDirs)
            {
                ...
                catch (Exception e)
                {
                    ...
                    Logger.WriteWarning(Logger.EventId.NotDefined, "Cannot delete the directory: " + dir, properties: new Dictionary<string, object> { { "Reason", e.Message }, { "StackTrace", e.StackTrace } });
                }
            }
        }
        ...
        private int[] ParseDefaultTopAndGrowth(string value)
        {
            ...
            for (int i = 0; i < items.Length; i++)
            {
                ...
                if (Int32.TryParse(items[i], out parsedInt))
                    ...
                    throw new ContentRepository.Storage.Data.ConfigurationException("Invalid sequence in the value of 'DefaultTopAndGrowth'. Every value can be positive integer except last, it can be positive integer or zero.");

                if (parsedInt < 0)
                    throw new ContentRepository.Storage.Data.ConfigurationException("Invalid sequence in the value of 'DefaultTopAndGrowth'. A value cannot less than 0.");

                if (parsedInt == 0)
                {
                    if(!last)
                        throw new ContentRepository.Storage.Data.ConfigurationException("Invalid sequence in the value of 'DefaultTopAndGrowth'. Only the last value can be 0.");
                }
                else
                {
                    if (i > 0 && parsedInt <= values[i - 1])
                        throw new ContentRepository.Storage.Data.ConfigurationException("Invalid sequence in the value of 'DefaultTopAndGrowth'. The sequence must be monotonically increasing. Last value can be greater than any other or zero.");
                }
            }
            return values;
        }
