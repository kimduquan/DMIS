...
namespace ContentRepository.Storage
{
    public class ApplicationInfo
    {
        ...
        public static ApplicationInfo CreateInitialVersion(string name, string edition, Version version, string description)
        {
            return ContentRepository.Storage.Data.DataProvider.Current.CreateInitialVersion(name, edition, version, description);
        }
    }
                ...
                lock (_typeCacheSync)
                {
                    if (!_typecacheByBase.TryGetValue(baseType, out temp))
                    {
                        ...
                        foreach (Assembly asm in GetAssemblies())
                        {
                            ...
                            {
                                Logger.WriteError(Logger.EventId.NotDefined, rtle.ToString(), properties: new Dictionary<string, object> { { "Assembly", asm.FullName }});
