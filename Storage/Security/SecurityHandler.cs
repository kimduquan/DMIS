...
namespace ContentRepository.Storage.Security
{
    ...
    public sealed class SecurityHandler
	{
        ...
        private static bool HasPermission(string path, int creatorId, int lastModifierId, params PermissionType[] permissionTypes)
        {
            ...
            return PermissionEvaluator.Instance.HasPermission(path.ToLowerInvariant(), user, isCreator, isLastModifier, permissionTypes);
        }
        ...
        private static bool HasSubTreePermission(string path, int creatorId, int lastModifierId, params PermissionType[] permissionTypes)
        {
            ...
            return PermissionEvaluator.Instance.HasSubTreePermission(path.ToLowerInvariant(), user, isCreator, isLastModifier, permissionTypes);
        }
        ...
        private static PermissionValue GetPermission(string path, int creatorId, int lastModifierId, params PermissionType[] permissionTypes)
        {
            ...
            var isCreator = user.Id == creatorId;
            var isLastModifier = user.Id == lastModifierId;
            return PermissionEvaluator.Instance.GetPermission(path.ToLowerInvariant(), user, isCreator, isLastModifier, permissionTypes);
        }
        ...
        private static PermissionValue GetSubtreePermission(string path, int creatorId, int lastModifierId, params PermissionType[] permissionTypes)
        {
            ...
            var isCreator = user.Id == creatorId;
            var isLastModifier = user.Id == lastModifierId; 
            return PermissionEvaluator.Instance.GetSubtreePermission(path.ToLowerInvariant(), user, isCreator, isLastModifier, permissionTypes);
        }
        ...
        private static PermissionValue[] GetAllPermissions(string path, int creatorId, int lastModifierId)
        {
            ...
            var isCreator = user.Id == creatorId;
            var isLastModifier = user.Id == lastModifierId;
            return PermissionEvaluator.Instance.GetAllPermissions(path.ToLowerInvariant(), user, isCreator, isLastModifier);
        }
        ...
        public static PermittedLevel GetPermittedLevel(string path, int creatorId, int lastModifierId)
        {
            ...
            bool isLastModifier = user.Id == lastModifierId;
            return PermissionEvaluator.Instance.GetPermittedLevel(path.ToLowerInvariant(), user, isCreator, isLastModifier);
        }
		...
		public SecurityEntry[] GetAllEntries()
		{
            ...
            return PermissionEvaluator.Instance.GetAllEntries(_node.Path.ToLowerInvariant());
		}
        public SecurityEntry[] GetExplicitEntries()
        {
            Assert(PermissionType.SeePermissions);
            return PermissionEvaluator.Instance.GetExplicitEntries(_node.Path.ToLowerInvariant());
        }
        public static SecurityEntry[] GetExplicitEntries(string path, int creatorId, int lastModifierId)
        {
            Assert(path, creatorId, lastModifierId, null, PermissionType.SeePermissions);
            return PermissionEvaluator.Instance.GetExplicitEntries(path.ToLowerInvariant());
        }

        public SecurityEntry[] GetEffectiveEntries()
        {
            return GetEffectiveEntries(_node.Path.ToLowerInvariant());
        }
        public static SecurityEntry[] GetEffectiveEntries(string path)
        {
            return GetEffectiveEntries(path.ToLowerInvariant(), null);
        }
        public static SecurityEntry[] GetEffectiveEntries(string path, IEnumerable<int> relatedIdentities)
        {
            ...
            return PermissionEvaluator.Instance.GetEffectiveEntries(path.ToLowerInvariant(), relatedIdentities);
        }
        public static SecurityEntry[] GetEffectiveEntries(string path, int creatorId, int lastModifierId)
        {
            ...
            return PermissionEvaluator.Instance.GetEffectiveEntries(path.ToLowerInvariant(), identities);
        }
        ...
        {
            Assert(path, creatorId, lastModifierId, null, PermissionType.SeePermissions);
            return PermissionEvaluator.Instance.GetAcl(nodeId, path.ToLowerInvariant(), creatorId, lastModifierId);
        }
        ...
        private static Exception GetAccessDeniedException(string path, int creatorId, int lastModifierId, string message, PermissionType[] permissionTypes, IUser user)
        {
            ...
            if (deniedPermission == null)
                throw new SecurityException(path, null, user);
            if (message != null)
                throw new SecurityException(path, deniedPermission, user, message);
            else
                throw new SecurityException(path, deniedPermission, user);
        }
        ...
        public void CopyPermissionsFrom(string sourcePath, CopyPermissionMode mode, bool reset = true)
        {
            ...
            var targetEntries = @clear ? new List<SecurityEntry>() : PermissionEvaluator.Instance.GetEffectiveEntries(_node.Path.ToLowerInvariant()).ToList();
            var sourceEntries = PermissionEvaluator.Instance.GetEffectiveEntries(sourcePath.ToLowerInvariant());
