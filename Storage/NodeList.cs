...
namespace ContentRepository.Storage
{
    ...
    public class NodeList<T> : INodeResolver<T>, IEnumerable<T>, /*IList<T>, System.Collections.IList,*/ IDynamicDataAccessor where T : Node
    {
        ...
        {
            var userId = ContentRepository.Storage.Security.AccessProvider.Current.GetCurrentUser().Id;
            if (userId < 0)
                return IdCount;
        ...
        /// <summary>
        /// Removes all elements from the <see cref="T:ContentRepository.Storage.NodeList`1"></see>, and then fires the Change event.
        /// </summary>
        public void Clear()
        {
            Modifying();
