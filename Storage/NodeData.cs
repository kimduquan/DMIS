...
namespace ContentRepository.Storage
{
    ...
    public class NodeData
    {
        ...
        internal enum StaticDataSlot
        {
            ...
            ParentId, Name, DisplayName, Path, Index, CreatingInProgress, IsDeleted, IsInherited,
            ...
            IsSystem, ClosestSecurityNodeId,
        ...
        internal int ClosestSecurityNodeId
        {
            get { return Get<int>(StaticDataSlot.ClosestSecurityNodeId); }
            set { Set<int>(StaticDataSlot.ClosestSecurityNodeId, value); }
        }
                ...
                foreach (var propTypeId in dynamicData.Keys.ToArray())
                {
                    ...
                    if (propType != null)
                    {
                        ...
                        if (propType.DataType == DataType.Text)
                        {
                            ...
                            if (!ContentRepository.Storage.Data.DataProvider.Current.IsCacheableText(item))
        ...
        private static readonly StaticDataSlot[] slotsToCopy = {
            ...
            //TODO: IsSystem, ClosestSecurityNodeId, SavingState, ChangedData
        };
        ...
        private string FormatDynamicData(object data, DataType dataType)
        {
            ...
            switch (dataType)
            {
                case DataType.Text:
                    ...
                    //    return text;
                    //return String.Concat("<![CDATA[", text.Replace("]]>", @"\]\]\>"), "]]>");

                    //var text = Convert.ToString(data, CultureInfo.InvariantCulture);
                    //if (string.IsNullOrWhiteSpace(text))
                    //    return string.Empty;
                    //return text.Replace("<![CDATA[", "<#![CDATA[").Replace("]]>", @"\]\]\>");
                case DataType.String:
                case DataType.Int:
                case DataType.Currency:
                ...
                case DataType.Binary:
                    ...
                    return String.Format(CultureInfo.InvariantCulture,
                              "Id: {0}, MimeType: {1}, FileName: {2}, IsEmpty: {3}, Size: {4}, Checksum {5}, Timestamp {6}",
                              bin.Id, bin.ContentType, bin.FileName, bin.IsEmpty, bin.Size, bin.Checksum, bin.Timestamp);
                case DataType.Reference:
