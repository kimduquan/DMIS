...
namespace ContentRepository
{
    ...
    public class Settings : File, ISupportsDynamicFields, ISupportsAddingFieldsOnTheFly
    {
            ...
            get
            {
                if (_binaryAsXml == null && !_xmlIsLoaded)
                {
                    if (this.Binary.Size > 0)
                    {
                        ...
                        {
                            Logger.WriteWarning(Logger.EventId.NotDefined, String.Concat("Error during deserializing setting binary to an XML document. Path: ", this.Path, " ", ex));
                        }
                    }
                    ...
                    if (this.Binary.Size > 0)
                    {
                        ...
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(binaryText) && !binaryText.StartsWith("<"))
                            {
                                ...
                                {
                                    deserialized = JObject.ReadFrom(jreader);
                                }

                                if (deserialized is JObject)
                                {
                                    _binaryAsJObject = (JObject)deserialized;
                                }
                                else if (deserialized is JArray && ((JArray)deserialized).Count > 0)
                                {
                                    _binaryAsJObject = ((JArray)deserialized)[0] as JObject;
                                }
                        ...
                        {
                            Logger.WriteWarning(Logger.EventId.NotDefined, "Error during deserializing setting binary to a JSON object. Path: " + this.Path + ". Error: " + ex.Message);
                        }
                    }
        ...
        public static T GetSettingsByName<T>(string settingsName, string contextPath) where T : Settings
        {
            ...
            if (!string.IsNullOrEmpty(contextPath) && !contextPath.Equals(Repository.RootPath))
            {
                // Try to load the settings content from the local Settings folder (e.g. '/Root/folder1/folder2/Settings/filename.settings').
                // Use the node only if it is really a settings content (cannot use Node.Load<T> here because it throws an exception in case of a type mismatch).
                var node = Node.LoadNode(RepositoryPath.Combine(contextPath, SETTINGSCONTAINERNAME, settingsName + "." + EXTENSION)) as T;

                return node ?? GetSettingsByName<T>(settingsName, RepositoryPath.GetParentPath(contextPath));
            }

            //load settings from the global settings folder
            return Node.Load<T>(RepositoryPath.Combine(SETTINGSCONTAINERPATH, settingsName + "." + EXTENSION));
        }
        ...
        public static T GetValue<T>(string settingsName, string key, string contextPath = null, T defaultValue = default(T))
        {
            using (new SystemAccount())
            {
                ...
                {
                    Logger.WriteWarning(Logger.EventId.NotDefined, "Settings file not found: " + settingsName + "." + EXTENSION);
                    return defaultValue;
                }
        ...
        protected virtual object GetValueFromXml(XmlNode xmlNode, string key)
        {
            ...
            if (xmlNode.HasChildNodes && !(xmlNode.FirstChild is XmlText))
                throw new NotImplementedException("Composite setting value parsing is not implemented.");
        ...
        public override void Save(NodeSaveSettings settings)
        {
            ...
            if (RepositoryInstance.LuceneManagerIsRunning && !RepositoryConfiguration.WorkingMode.Importing)
            {
        ...
        private void AssertSettingsPath(string path, string name, int id)
        {
            if (!path.Contains(SETTINGSCONTAINERNAMEPART) &&
                ...
                !path.StartsWith(RepositoryPath.Combine(Repository.ContentTemplateFolderPath, RepositoryPath.PathSeparator), StringComparison.OrdinalIgnoreCase) &&
            ...
            //check name (settings content in the settings folder with the same name)
            ...
            else
            {
                ...
                var nqResult = NodeQuery.QueryNodesByTypeAndPathAndName(ActiveSchema.NodeTypes["Settings"], false, rootpath, false, name);
        ...
        private T GetValueFromXmlInternal<T>(XmlNode xmlNode, string key)
        {
            ...
            else if (tt.IsEnum)
            {
                ...
                {
                    Logger.WriteWarning(Logger.EventId.NotDefined, "Unknown value for enum type. Value: " + stringValue + ", Type: " + tt.FullName + ". Settings: " + this.Path);
                ...
                throw new NotImplementedException("Not supported settings type: " + tt.Name);
        ...
        private T GetValueFromJsonInternal<T>(JToken token, string key)
        {
            ...
            {
                Logger.WriteWarning(Logger.EventId.NotDefined, string.Format("Error during setting value JSON conversion. Path: {0}. Key: {1}. Expected type: {2}. Exception: {3}", this.Path, key, typeof(T).FullName, ex.Message));
            }
        ...
        public override IEnumerable<IIndexableField> GetIndexableFields()
        {
            ...
            // The problem is that can't handle two differently typed fields with the same name.
    ...
    //public class SettingsObserver : NodeObserver
    //{
    //    [Serializable]
    //    internal sealed class SettingsInvalidatorDistributedAction : DistributedAction
    //    {
    //        public string Path { get; private set; }

    //        public SettingsInvalidatorDistributedAction(string path)
    //        {
    //            Path = path;
    //        }
    ...
    //        {
    //            //Settings.InvalidateSettings(this.Path);
    //        }
    //    }
    ...
    //    {
    //        if (!IsSettingContent(e.SourceNode))
    //            return;

    //        var da = new SettingsInvalidatorDistributedAction(e.SourceNode.Path);
    //        da.Execute();
    ...
    //    private static bool IsSettingContent(Node node)
    //    {
    //        return node != null &&
    //            node.InTree(Settings.SETTINGSCONTAINERPATH + RepositoryPath.PathSeparator) &&
    //            node.TypeIs("Settings");
    //    }
    //}
}
