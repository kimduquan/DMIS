...
namespace ContentRepository
{
    internal class ApplicationCache : IApplicationCache
    {
        ...
        public void Invalidate(string appTypeName, string path)
        {
            if (locked)
                return;
            Logger.WriteVerbose("ApplicationCache is invalidated.", null, new Dictionary<string, object> { { "AppTypeName", appTypeName }, { "Path", path } });
        ...
        private List<string> SearchData(string appTypeName)
        {
            ...
            Logger.WriteVerbose("ApplicationCache is created.", null, new Dictionary<string, object> { { "AppTypeName", appTypeName }, { "Count", data.Count } });
            return data;
        }
    }
}
