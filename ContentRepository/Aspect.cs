...
{
    //[ContentHandler]
    //public class Aspect : ContentList
    //{
    ...
    //    protected override ContentListType ManageContentListType(Dictionary<string, FieldDescriptor> fieldInfoList, Dictionary<string, List<string>> oldBindings, bool modify, out List<FieldSetting> fieldSettings)
    //    {
    ...
    //                fieldSettings.Add(FieldSetting.Create(fieldInfoList[name], null /*oldBindings[name]*/));
    ...
    //            //-- new
    //            foreach (string name in fieldInfoList.Keys)
    //                fieldSettings.Add(CreateNewFieldType(fieldInfoList[name]));
    //            hasChanges = true;
    //        }
    //        else
    //        {
    //            //-- merge
    //            hasChanges |= RemoveUnusedFields(fieldInfoList, oldBindings);
    //            foreach (string name in fieldInfoList.Keys)
    //            {
    ...
    //        return null;
    //    }
    //    private bool RemoveUnusedFields(Dictionary<string, FieldDescriptor> fieldInfoList, Dictionary<string, List<string>> oldBindings)
    //    {
    //        bool hasChanges = false;
    //        for (int i = _fieldSettings.Count - 1; i >= 0; i--)
    //        {
    //            if (!fieldInfoList.ContainsKey(_fieldSettings[i].Name))
    //            {
    //                hasChanges = true;
    ...
    //    {
    //        if (!fieldInfo.FieldName.StartsWith("#_"))
    //            throw new InvalidOperationException("Aspect field name must start with '#_'. Bad name: " + fieldInfo.FieldName);
    //        return FieldSetting.Create(fieldInfo, new List<string>());
    //    }
    ...
    {
        public static readonly string AspectDefinitionXmlNamespace = "http://schemas.com/ContentRepository/AspectDefinition";
        private static string AspectDefinitionSchemaManifestResourceName = "ContentRepository.Schema.AspectDefinition.xsd";
            ...
            foreach (XPathNavigator subElement in rootElement.SelectChildren(XPathNodeType.Element))
            {
                switch (subElement.LocalName)
                {
                    ...
                    case "Actions":
                        ...
                        Logger.WriteWarning(Logger.EventId.NotDefined, "Ignoring obsolete Actions element in List definition: " + this.Name);
                        break;
                    default:
                        throw new NotSupportedException(String.Concat("Unknown element in AspectDefinition: ", subElement.LocalName));
                }
            }
            return result;
        }
        ...
        {
            if (StorageContext.Search.IsOuterEngineEnabled && StorageContext.Search.SearchEngine != ContentRepository.Storage.Search.InternalSearchEngine.Instance)
        ...
        public void AddFields(params ContentRepository.Schema.FieldInfo[] fieldInfos)
        {
