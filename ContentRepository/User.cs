    ...
    public class User : GenericContent, IUser, IADSyncable
    {
        ...
        public static IUser Current
        {
            ...
            set // [Explicit SignIn]
            {
                ...
                if (value.Id == 0)
                    throw new SecurityException("Cannot log in with a non-saved (non-existing) user.");
        ...
        {
            if (!Repository.UserProfilesEnabled || string.IsNullOrEmpty(oldUsername))
                return;
        ...
        private void CheckUniqueUser()
        {
            ...
            if (!path.StartsWith(string.Concat(Repository.ImsFolderPath, RepositoryPath.PathSeparator)) || ParentPath == Repository.ImsFolderPath)
            {
            ...
            if (RepositoryInstance.ContentQueryIsAllowed)
            {
                ...
                using (new SystemAccount())
                {
                    var queryResult = ContentQuery.Query(SafeQueries.UserOrGroupByName,
                        new QuerySettings {EnableAutofilters = FilterStatus.Disabled, Top = 2},
                        domainPath.TrimEnd('/'), Name);

                    identifiers = queryResult.Identifiers;
