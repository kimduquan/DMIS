...
namespace Search.Indexing
{
    public class MissingActivityHandler
    {
        ...
        static MissingActivityHandler()
        {
            ...
            GapSegments = ContentRepository.Storage.Data.RepositoryConfiguration.IndexHealthMonitorGapPartitions;
        ...
        internal static void RemoveActivityAndAddGap(int activityId)
        {
            ...
            try
            {
                ...
                Search.Indexing.Activities.DistributedLuceneActivity.WriteMSMQLog(activityId, Search.Indexing.Activities.DistributedLuceneActivity.MSMQLogStatusType.REMOVED, null);
                ...
                if (!removed)
                {
                    ...
                    if (activityId - 1 > _maxActivityId)
                    {
                        ...
                        {
                            _currentGap.Add(i);
                            Search.Indexing.Activities.DistributedLuceneActivity.WriteMSMQLog(i, Search.Indexing.Activities.DistributedLuceneActivity.MSMQLogStatusType.ADDED, activityId);
                        }
                    }
                }
        ...
        internal static bool RemoveActivityFromGap(int activityId)
        {
            _gapRWSync.EnterWriteLock();

            try
            {
                for (var i = _currentGapIndex + GapSegments; i > _currentGapIndex; i--)
                {
                    if (_gaps[i % GapSegments].Remove(activityId))
                        return true;
                }
