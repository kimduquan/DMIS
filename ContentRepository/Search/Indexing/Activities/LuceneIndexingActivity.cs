...
namespace Search.Indexing.Activities
{
    ...
    internal abstract class LuceneIndexingActivity : DistributedLuceneActivity
    {
        ...
        public static T CreateFromIndexingActivity<T>(IndexingActivity activity) where T : LuceneIndexingActivity, new()
        {
            ...
            result.FromExecutingUnprocessedActivities = activity.FromExecutingUnprocessedActivities;
        ...
        {
            // unregisteractivity must be called after executing an activity, even if index is not changed
            LuceneManager.UnregisterActivity(this.ActivityId, this.FromExecutingUnprocessedActivities);
        }
    }
}