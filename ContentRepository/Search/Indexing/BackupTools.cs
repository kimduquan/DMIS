...
using System.Xml;
using StorageContext = ContentRepository.Storage.StorageContext;
using Diagnostics;
...
<RestoreInfo>
  <BackupId>{0}</BackupId>
        ...
        {
            get { return ContentRepository.Storage.IndexDirectory.CurrentDirectory; }
        }
        ...
        public static void BackupIndex()
        {
            ...
            var activity = new Search.Indexing.Activities.BackupActivity(Environment.MachineName, AppDomain.CurrentDomain.FriendlyName);
            activity.Distribute();
        ...
        internal static void OptimizeCompressAndStore() // caller: BackupIndexImmediatelly (synchron), BackupActivity.Execute() (asynchron)
        {
            ...
            Logger.WriteInformation(Logger.EventId.NotDefined, "Index directory is successfully backed up.");
        }
        ...
        internal static void RestoreIndex(bool force, System.IO.TextWriter consoleOut)
        {
            using (var optrace = new OperationTrace("Restore index"))
            {
                ...
                if (force || need)
                {
                    ...
                    Logger.WriteInformation(EventId.Indexing.SuccessfullyRestored, "Index directory is successfully restored.", properties: new Dictionary<string, object> { { "BackupId", lastIdFromDb } });
                }
                else
                {
                    if (consoleOut != null)
                        consoleOut.WriteLine("    Index directory restoring is skipped.");
                    Logger.WriteVerbose("Index directory restoring is skipped.", new Dictionary<string, object> { { "BackupId", lastIdFromDb } });
                }
                optrace.IsSuccessful = true;
            }
        }
        ...
        private static bool NeedRestore(out Guid lastIdFromDb)
        {
            ...
            lastIdFromDb = ContentRepository.Storage.DataBackingStore.GetLastStoredBackupNumber();
        ...
        private static void OptimizeBeforeBackup(string indexDirectoryPath)
        {
            using (var optrace = new OperationTrace("Optimize index"))
            {
                ...
                writer.Optimize();
                writer.Close();
                if (!ContentRepository.RepositoryInstance.WaitForWriterLockFileIsReleased(StorageContext.Search.IndexDirectoryBackupPath))
        ...
        {
            ContentRepository.Storage.DataBackingStore.RecoverIndexBackupFromDb(recoveredFilePath);
        }
    }
}
