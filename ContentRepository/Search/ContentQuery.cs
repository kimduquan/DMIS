...
using ContentRepository.Search;
...
namespace Search
{
    ...
    public class ContentQuery : IContentQuery
    {
        ...
        static ContentQuery()
        {
            ...
            foreach (Type t in TypeHandler.GetTypesByInterface(typeof(ISafeQueryHolder)))
            {
                    ...
                    if (__escaperRegex == null)
                    {
                        lock (__escaperRegexSync)
                        {
                            if (__escaperRegex == null)
                            {
                                ...
                                foreach (var c in Search.Parser.SnLucLexer.STRINGTERMINATORCHARS.ToCharArray())
                                    pattern.Append("\\" + c);
                                pattern.Append("]");
