                ...
                if ((_current == null) && (!_isInitialized))
                {
                    lock (_lock)
                    {
                        if ((_current == null) && (!_isInitialized))
                        {
                            ...
                            {
                                throw new ConfigurationErrorsException(String.Concat(SR.Exceptions.Configuration.Msg_DirectoryProviderImplementationDoesNotExist, ": ", DirectoryProviderClassName));
                            }
                            catch (InvalidCastException) //rethrow
                            {
                                throw new ConfigurationErrorsException(String.Concat(SR.Exceptions.Configuration.Msg_InvalidDirectoryProviderImplementation, ": ", DirectoryProviderClassName));
                            }
                            finally
                            {
                                _isInitialized = true;
                            ...
                            if(_current == null)
                                Logger.WriteInformation(Logger.EventId.NotDefined, "DirectoryProvider not present.");
                            else
                                Logger.WriteInformation(Logger.EventId.NotDefined, "DirectoryProvider created: " + _current.GetType().FullName);
                        }
                    }
                }
                return _current;
