...
namespace ApplicationModel
{
    ...
    public sealed class ApplicationStorage
    {
        ...
        private List<Application> GetApplicationsInternal2(string appName, NodeHead head, string scenarioName, string requestedDevice)
        {
            ...
            var device = requestedDevice == null ? new string[0] : DeviceManager.GetDeviceChain(requestedDevice.ToLowerInvariant());
...
timer = Stopwatch.StartNew();
            ...
            if (appName != null)
                requestedNames = new List<string>(new[] { appName.ToLowerInvariant() });
            else
                requestedNames =  _appNames.Select(s => s.ToLowerInvariant()).ToList();
            ...
            foreach (var parentPath in paths)
            {
                ...
                {
                    var lowerName = app.AppName.ToLowerInvariant();
...
if (!(ex is NotImplementedException))
    ...
    public sealed class ApplicationStorage
    {
        ...
        private static void InvalidatePrivate()
        {
            Logger.WriteInformation(Logger.EventId.NotDefined, "ApplicationStorage invalidate");
            _instance = null;
        }
