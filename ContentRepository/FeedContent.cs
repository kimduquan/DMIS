...
namespace ContentRepository
{
    public abstract class FeedContent
    {
        ...
        protected void WriteHead(XmlWriter writer, string contentTypeName, string contentName, string iconName, string path, bool isFolder)
        {
            ...
            var ctDisplayName = ct == null ? contentTypeName : ResourceManager.Current.GetString(ct.DisplayName);
            ...
            foreach (var action in actions)
            {
                try
                {
                    if (action.Active)
                    {
                        ...
                        {
                            Logger.WriteWarning(Logger.EventId.NotDefined, String.Concat("Content actions serialization error: invalid xml element. Path: ", path, ". Action name: ", action.Name));
                            continue;
                        }
