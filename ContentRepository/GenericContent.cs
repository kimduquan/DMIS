...
namespace ContentRepository
{
    ...
    public class GenericContent : Node, IIndexableDocument
    {
        ...
        internal void AssertAllowedChildType(Node node, bool move = false)
        {
            switch (CheckAllowedChildType(node))
            {
                ...
                case TypeAllow.TypeIsNotPermitted:
                    throw new SecurityException(node.Path, PermissionType.See, User.Current);
        ...
        public virtual void Delete(bool bypassTrash)
        {
            ...
            DetailedLogger.Log(op, "GC.Delete: VId:{0}, Path:{1}", this.VersionId, this.Path);
#endif
        ...
        public virtual void Save(SavingMode mode)
        {
            ...
            DetailedLogger.Log(op, "GC.Save: Mode:{0}, VId:{1}, Path:{2}", mode, this.VersionId, this.Path); // category: INDEX
#endif
...
#if INDEX
            var op = DetailedLogger.CreateOperation(); // category: INDEX
            DetailedLogger.Log(op, "GC.CheckOut: VId:{0}, Path:{1}", this.VersionId, this.Path); // category: INDEX
#endif
...
#if INDEX
            var op = DetailedLogger.CreateOperation(); // category: INDEX
            DetailedLogger.Log(op, "GC.CheckIn: VId:{0}, Path:{1}", this.VersionId, this.Path); // category: INDEX
#endif
            var action = SavingAction.Create(this);
            action.CheckIn();
            action.Execute();
...
#if INDEX
            var op = DetailedLogger.CreateOperation(); // category: INDEX
            DetailedLogger.Log(op, "GC.UndoCheckOut: forceRefresh:{0}, VId:{1}, Path:{2}", forceRefresh, this.VersionId, this.Path); // category: INDEX
#endif
...
#if INDEX
            var op = DetailedLogger.CreateOperation(); // category: INDEX
            DetailedLogger.Log(op, "GC.Publish: VId:{0}, Path:{1}", this.VersionId, this.Path); // category: INDEX
#endif
            var action = SavingAction.Create(this);
            action.Publish();
            action.Execute();
...
#if INDEX
            var op = DetailedLogger.CreateOperation(); // category: INDEX
            DetailedLogger.Log(op, "GC.Approve: VId:{0}, Path:{1}", this.VersionId, this.Path); // category: INDEX
#endif
            var action = SavingAction.Create(this);
            action.Approve();
            action.Execute();
...
#if INDEX
            var op = DetailedLogger.CreateOperation(); // category: INDEX
            DetailedLogger.Log(op, "GC.Reject: VId:{0}, Path:{1}", this.VersionId, this.Path); // category: INDEX
#endif
            var action = SavingAction.Create(this);
            action.Reject();
            action.Execute();
...
#if INDEX
            var op = DetailedLogger.CreateOperation(); // category: INDEX
            DetailedLogger.Log(op, "GC.FinalizeContent: SavingState:{0}, VId:{1}, Path:{2}", this.SavingState, this.VersionId, this.Path); // category: INDEX
#endif
