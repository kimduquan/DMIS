...
namespace ContentRepository
{
    ...
    public class TrashBag : Folder
    {
        ...
        private static void CopyPermissions(Node source, Node target)
        {
            ...
            target.Security.CopyPermissionsFrom(source.Path, CopyPermissionMode.BreakAndClear, false);
            ...
            foreach (var securityEntry in source.Security.GetEffectiveEntries().Where(s => s.PrincipalId == RepositoryConfiguration.CreatorsGroupId))
            {
                target.Security.SetPermissions(source.CreatedById, true, securityEntry.PermissionValues);
            }
        }
