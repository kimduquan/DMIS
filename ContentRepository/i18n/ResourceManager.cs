...
namespace ContentRepository.i18n
{
    public class ResourceManagerBridge : IResourceManager
    {
        public bool Running
        {
            get { return ResourceManager.Running; }
        }
        ...
        {
            return String.Format(ResourceManager.Current.GetString(fullResourceKey), args);
        }

        public bool ParseResourceKey(string source, out string className, out string name)
        {
            return ResourceManager.ParseResourceKey(source, out className, out name);
        }

        public IEnumerable<string> GetResourceFilesForClass(string className)
        {
            return ResourceManager.Current.GetResourceFilesForClass(className);
        }
    }
    ...
    public sealed class ResourceManager
    {
        ...
        internal sealed class ResourceManagerResetDistributedAction : DistributedAction
        {
            public override void DoAction(bool onRemote, bool isFromMe)
            {
                ...
                if (onRemote && isFromMe)
                    return;
                ResourceManager.ResetPrivate();
            }
        }

        internal static void Reset()
        {
            Logger.WriteInformation(EventId.Resource.ManagerReset, "ResourceManager.Reset called.",
        ...
        {
            Logger.WriteInformation(EventId.Resource.ManagerReset, "ResourceManager.Reset executed.",
                properties: new Dictionary<string, object> { { "AppDomain", AppDomain.CurrentDomain.FriendlyName } });
        ...
        private static ResourceManager _current;
            ...
            get
            {
                if (_current == null)
                {
                    lock (_syncRoot)
                    {
                        if (_current == null)
                        {
                            var current = new ResourceManager();
                            current.Load();
                            _current = current;
                            Logger.WriteInformation(Logger.EventId.NotDefined, "ResourceManager created: " + _current.GetType().FullName);
                        }
                    }
                }
        ...
        Dictionary<string, List<string>> _classFiles = new Dictionary<string, List<string>>();
        ...
        public static bool IsResourceEditorAllowed
        {
            get
            {
                ...
                return HttpContext.Current.Request.Cookies.AllKeys.Contains("AllowResourceEditorCookie") && User.Current.IsInGroup(Group.Administrators);
            }
        }
