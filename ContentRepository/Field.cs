...
{
    /// <summary>
    /// <c>Field</c> represent an atomic data of a <see cref="ContentRepository.Content">Content</see>
    /// </summary>
    /// <remarks>
    /// 
    /// A <c>Field</c> represents an atomic data type from the perspective of the system.
    /// It usually handles <c>Fields</c> usually handles a single <see cref="ContentRepository.Storage.Node">ContentHandler</see> property.
    /// It is also capable of handling of multiple <see cref="ContentRepository.Storage.Node">ContentHandler</see> properties (see the <see cref="Portal.UI.Controls.WhoAndWhen">WhoAndWhen</see> Field for an example).
    /// It may also consist of values computed from the handled <see cref="ContentRepository.Storage.Node">ContentHandler</see> propery(s) or even represent data independent of these properties.
    /// 
    ...
    /// The Field constructs a transfer object from ContentHandler properties and passes it up to the <see cref="Portal.UI.Controls.FieldControl">FieldControl</see>.
    ...
    /// 
	/// The <c>Field</c> class is responsible for invoking data validation (data validation itself is done by the  <see cref="ContentRepository.Schema.FieldSetting">FieldSetting</see> property).
    /// 
    /// The class <c>Field</c> can be resembled as bridge between <see cref="ContentRepository.Storage.Node">ContentHandler</see>properties and <see cref="Portal.UI.ContentView">ContentView</see> (or any other ways of data output/input).
    /// 
    /// The <c>Field</c> class is also one of the most important system extension points (among with <see cref="ContentRepository.Storage.Node">ContentHandler</see>).
	...
	public abstract partial class Field : IIndexableField
	{
        ...
        /// <summary>
        /// Gets a backreference to the  <see cref="ContentRepository.Content">Content</see> referencing the Field
        /// </summary>
		public Content Content { get; private set; }
        /// <summary>
		/// Gets approriate <see cref="ContentRepository.Schema.FieldSetting">FieldSetting</see> for the Field which is configured in the Field/Cofiguration block in the CTD
        /// </summary>
        /// <remarks>
        /// By default that <see cref="ContentRepository.Schema.FieldSetting">FieldSetting</see> is assigned to the Field which is defined in the [DefaultFieldSetting] attribte on the overriden Field class.
		...
		{
            if (!LocalizationEnabled || !localized || !ResourceManager.Running)
                return Value;
            ...
            if (ResourceManager.ParseResourceKey(stringData, out className, out name))
        ...
        public bool IsLocalized
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(sdata))
                    return false;
                return sdata[0] == ResourceManager.ResourceKeyPrefix;
            }
        }
        ...
        {
            if (!LocalizationEnabled || !ResourceManager.Running)
            ...
            if (ResourceManager.ParseResourceKey(stringData, out className, out name))
            {
                if (cultureInfo == null)
                    return ResourceManager.Current.GetString(className, name);
                return ResourceManager.Current.GetString(className, name, cultureInfo);
            }
            return stringData;
        }
        ...
        /// <summary>
        /// Creates a transfer object from <see cref="ContentRepository.Storage.Node">ContentHandler</see> properties
        /// </summary>
        ...
        /// <summary>
        /// Creates <see cref="ContentRepository.Storage.Node">ContentHandler</see> properties from the transfer object.
        /// </summary>
