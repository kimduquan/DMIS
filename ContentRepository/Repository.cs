...
namespace ContentRepository
{
    public static class Repository
    {
        ...
        /// };
        /// using (ContentRepository.Repository.Start(startSettings))
        /// {
        ///     // your code
        /// }
        /// </code>
        ...
        [ApplicationModel.ODataFunction]
        ...
        public static readonly string PORTALSECTIONKEY = "dmis/portalSettings";
        ...
        private static object _creationSync = new object();
        ...
        public static CheckInCommentsMode CheckInCommentsMode
        {
            get
            {
                var settings = ConfigurationManager.GetSection("dmis/portalSettings") as NameValueCollection;
                if (settings != null)
                {
                ...
                if (_webdavExtensions == null)
                {
                    lock (_webdavExtensionsLocker)
                    {
                        if (_webdavExtensions == null)
                        {
                            ...
                            //not found in config
                            if (_webdavExtensions == null)
                                _webdavExtensions = new[] { ".doc", ".docx", ".xls", ".xlsx", ".xlsm", ".xltx", ".ods", ".odt", ".odp", ".ppt", ".pptx", ".pps", ".ppsx", ".rtf", ".mpp" }; 
                        }
                    }
                }
        ...
        private static string[] _downloadExtensions;
        private static object _downloadExtensionsLocker = new object();
        public static string[] DownloadExtensions
        {
            get
            {
                if (_downloadExtensions == null)
                {
                    lock (_downloadExtensionsLocker)
                    {
                        if (_downloadExtensions == null)
                        {
                            var extList = WebdavEditExtensions.ToList();
                            if (extList.All(e => e.ToLower() != ".pdf"))
                                extList.Add(".pdf");

                            _downloadExtensions = extList.ToArray();
                        }
                    }
                }
        ...
        private const string IndexingPausedTimeoutKey = "IndexingPausedTimeout";
        private static int? _indexingPausedTimeout;
        public static int IndexingPausedTimeout
        {
            get
            {
                if (!_indexingPausedTimeout.HasValue)
                    _indexingPausedTimeout = GetIntegerConfigValue(IndexingPausedTimeoutKey, 60);

                return _indexingPausedTimeout.Value;
            }
        }

        private static readonly string[] _executableExtensions = { "aspx", "ashx", "asmx", "axd", "cshtml", "vbhtml" };
        public static string[] ExecutableExtensions { get { return _executableExtensions; } }
