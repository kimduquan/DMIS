...
namespace ContentRepository.Linq
{
    internal class CQVisitor : ExpressionVisitor
    {
        private class ContentHandlerGetTypeQuery : Query
        {
            ...
            {
                throw new NotImplementedException("SnLinq: ContentHandlerGetTypeQuery.ToString");
            }
        }
        internal class BooleanMemberQuery : Query
        {
            ...
            public override string ToString(string field)
            {
                throw new NotImplementedException("SnLinq: BooleanMemberQuery.ToString");
            }
        }
        ...
        internal Query GetQuery(Type sourceCollectionItemType, ChildrenDefinition childrenDef)
        {
            if (_queries.Count == 0)
                return null;
            if (_queries.Count > 1)
                throw new Exception("Stack contains more than one queries.");
        ...
        private void Trace(MethodBase methodBase, object node)
        {
            ...
            var nodeType = expr == null ? node.GetType().Name : expr.NodeType.ToString();

            if (nodeType == "Call")
                nodeType += " " + ((MethodCallExpression)node).Method.Name;

            if (node == null)
                Console.WriteLine("{0} (?): [null]", methodBase.Name);
            else
                Console.WriteLine("{0} ({1}): {2}", methodBase.Name, nodeType, node);

            if (node == null)
                Debug.WriteLine("{0} (?): [null]", methodBase.Name);
            else
                Debug.WriteLine("{0} ({1}): {2}", methodBase.Name, nodeType, node);
        }
        ...
        protected override Expression VisitBinary(BinaryExpression node)
        {
            ...
            switch (visited.NodeType)
            {
                ...
                */
                default: throw new NotImplementedException("SnLinq: VisitBinary");
            }
            return visited;
        }
        ...
        protected override Expression VisitMember(MemberExpression node)
        {
            ...
            if (memberExpr.Type == typeof(bool))
                _queries.Push(new BooleanMemberQuery { FieldName = memberExpr.Member.Name, Value = true });
        ...
        protected override Expression VisitMethodCall(MethodCallExpression node)
        {
            ...
            switch (methodCallExpr.Method.Name)
            {
                ...
                case "substringof":
                    {
                        var fieldName = GetPropertyName(methodCallExpr.Arguments[0]);
                        var substringofExpr = GetArgumentAsConstant(methodCallExpr, 1);
                ...
                default:
                    throw new NotImplementedException("Unknown method: " + methodCallExpr.Method.Name);
            }
        ...
        protected override Expression VisitUnary(UnaryExpression node)
        {
            ...
            if (unary.NodeType == ExpressionType.Convert)
            {
                if (unary.Type == typeof(bool))
                {
                    ...
                    _queries.Push(new BooleanMemberQuery { FieldName = name, Value = true });
                }
            }
        ...
        Stack<Query> _queries = new Stack<Query>();
        ...
        private bool TopLevelQueryIsBooleanMemberQuery()
        {
            if (_queries.Count == 0)
                return false;
            return _queries.Peek() is BooleanMemberQuery;
        }
        ...
        private void BuildFieldExpr(BinaryExpression node)
        {
            ...
            switch (node.NodeType)
            {
                ...
                case ExpressionType.LessThanOrEqual:     // field:[ to value]
                    ...
                    break;
                default:
                    throw new NotImplementedException(string.Format("NodeType {0} isn't implemented", node.NodeType));
            }
            if (node.Type == typeof(bool))
                if (TopLevelQueryIsBooleanMemberQuery())
                    _queries.Pop();
            _queries.Push(query);
        }
        ...
        private static Term CreateTerm(string name, QueryFieldValue queryFieldValue)
        {
            ...
            switch (queryFieldValue.Datatype)
            {
                ...
                default:
                    throw new NotImplementedException("Unknown IndexableDataType enum value: " + queryFieldValue.Datatype);
            }
        ...
        private static QueryFieldValue ConvertValue(string name, object value, out Type fieldDataType)
        {
            ...
            var fieldInfo = ContentRepository.Schema.ContentTypeManager.GetPerFieldIndexingInfo(name);
            if (fieldInfo == null)
        ...
        private Query CreateRangeQuery(string fieldName, QueryFieldValue minValue, QueryFieldValue maxValue, bool includeLower, bool includeUpper)
        {
            ...
            switch (dataType)
            {
                ...
                default:
                    throw new NotImplementedException("Unknown IndexableDataType: " + dataType);
            }
        }
        ...
        {
            var fieldInfo = ContentRepository.Schema.ContentTypeManager.GetPerFieldIndexingInfo(field);
            if (fieldInfo == null)
            ...
            switch (type)
            {
                ...
                default: throw new NotImplementedException("WildcardType is not implemented: " + type);
            }
            ...
            if (methodCallExpr != null)
            {
                if (methodCallExpr.Method.Name == "get_Item" && methodCallExpr.Object.Type == typeof(Content))
                {
                    ...
                    if (constantArgumentExpr != null)
                        ...
                        throw new NotImplementedException("SnLinq: GetPropertyName from Content indexer expression");
                }
                else
                {
                    throw new NotImplementedException("SnLinq: GetPropertyName from MethodCallExpression");
                }
            }
            throw new NotImplementedException("SnLinq: GetPropertyName from Expression");
        }
