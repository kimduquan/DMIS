...
using ContentRepository.Storage.Scripting;
using System.IO;
using System.Web;
...
namespace ContentRepository.Schema
{
    ...
    /// </summary>
    /// <remarks>
    /// The <c>FieldSetting</c> is basically the validation logic for a <see cref="ContentRepository.Field">Field</see> object.
    /// 
    /// Looking at the big picture, a Content Type Definition defines a Content Type by defining <see cref="ContentRepository.Field">Field</see>s (within the Field tag of the CTD) and FieldSettings to each fields (within the Field/Configuration element in the CTD)
    /// A ContentType is therefore a collection of Fields with a FieldSetting assigned to each.
    /// 
    /// FieldSettings by default are assigned to <see cref="ContentRepository.Field">Field</see>s automatically (e.g. <see cref="ContentRepository.Fields.ShortTextFieldSetting">ShortTextFieldSetting</see> is assigned to <see cref="Portal.UI.Controls.ShortText">ShortText</see>).
    /// However, a custom FieldSetting can be assigned to a <see cref="ContentRepository.Field">Field</see> by specifying the handler attribute of the Field/Configuration element.
    /// </remarks>
    /// 
    ...
    public abstract class FieldSetting
    {
        ...
        public string DisplayName
        {
            get
            {
                if (_displayName != null)
                {
                    ...
                    return ResourceManager.ParseResourceKey(_displayName, out className, out name)
                        ? ResourceManager.Current.GetString(className, name)
                        : _displayName;
                }
        ...
        public string Description
        {
            get
            {
                ...
                {
                    string className, name;
                    return ResourceManager.ParseResourceKey(_description, out className, out name)
                        ? ResourceManager.Current.GetString(className, name)
                        : _description;
                }
        ...
        /// Visually:
        /// ContentType parent----ParentFieldSetting (e.g. <see cref="Portal.UI.Controls.ShortText">ShortText</see>)
        ///     ^
        ///     |
        ///     |
        /// ContentType owner-----FieldSetting (e.g. <see cref="Portal.UI.Controls.ShortText">ShortText</see>)
        /// </remarks>
        ...
        static IndexStoringMode ToIndexStoringMode(LucField.Store s)
        {
            if (s == null)
        ...
        static IndexingMode ToIndexingMode(LucField.Index m)
        {
            if (m == null)
        ...
        static IndexTermVector ToIndexTermVector(LucField.TermVector v)
        {
            if (v == null)
        ...
        public FieldInfo ToFieldInfo()
        {
            ...
            fieldInfo.Configuration.Handler = this.GetType().Name;
            ...
            fieldInfo.Indexing.IndexHandler = this.IndexingInfo.IndexFieldHandler.GetType().Name;
        ...
        private void Parse(XPathNavigator nav, IXmlNamespaceResolver nsres, ContentType contentType)
        {
            ...
            _outputMethod = iter.MoveNext() ? (OutputMethod?)Enum.Parse(typeof(ContentRepository.Schema.OutputMethod), iter.Current.InnerXml, true) : null;
        ...
        private static void SetProperties(FieldSetting setting, FieldInfo descriptor, List<string> bindings)
        {
            ...
            //        throw new InvalidOperationException("FieldInfo.Type cannot be null if the FieldInfo.Handler is null.");
            //    fieldTypeName = FieldManager.GetFieldHandlerName(descriptor.Type);
            //}
        ...
        {
            if (this.IndexingInfo != null)
            {
                // <Indexing>
                writer.WriteStartElement("Indexing");

                if (this.IndexingInfo.IndexingMode != null)
                    WriteElement(writer, ToIndexingMode(this.IndexingInfo.IndexingMode).ToString(), "Mode");
                if (this.IndexingInfo.IndexStoringMode != null)
                    WriteElement(writer, ToIndexStoringMode(this.IndexingInfo.IndexStoringMode).ToString(), "Store");
                if (this.IndexingInfo.TermVectorStoringMode != null)
                    WriteElement(writer, ToIndexTermVector(this.IndexingInfo.TermVectorStoringMode).ToString(), "TermVector");
                if (!string.IsNullOrEmpty(this.IndexingInfo.Analyzer))
                    WriteElement(writer, this.IndexingInfo.Analyzer, "Analyzer");
                if (this.IndexingInfo.IndexFieldHandler != null)
                    WriteElement(writer, this.IndexingInfo.IndexFieldHandler.GetType().FullName, "IndexHandler");
        ...
        /// <summary>
        /// Infers a  field setting from a given XML which is ready to use in the system, using the given name.
        /// </summary>
        ...
        /// <summary>
        /// Infers a  field setting from a given string which is ready to use in the system, using the given name.
        /// </summary>
        ...
        /// <summary>
        /// Infers a  field setting from a given type which is ready to use in the system, using the given name.
        /// </summary>
