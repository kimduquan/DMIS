...
namespace ContentRepository.Mail
{
                ...
                if (_instance == null)
                {
                    lock (_mailProviderLock)
                    {
                        if (_instance == null)
                        {
                            try
                            {
                                var mailProviderTypes = TypeHandler.GetTypesByBaseType(typeof(MailProvider))
                                ...
                                if (mailProviderTypes.Length > 0)
                                {
                                    ...
                                    {
                                        Logger.WriteWarning(Logger.EventId.NotDefined, "Too many mailprovider implementations exist, there must be only one. Fallback to default provider.", 
                                            properties: new Dictionary<string, object> { { "Mailproviders", string.Join(", ", mailProviderTypes.Select(t => t.FullName))}});
                                    }
                                    else
                                    {
                                        _instance = (MailProvider)TypeHandler.CreateInstance(mailProviderTypes.First().FullName); 
                                    }
                                }
                            }
                            catch (Exception ex)
                            ...
                            if (_instance == null)
                                _instance = (MailProvider)TypeHandler.CreateInstance(typeof(DefaultMailProvider).FullName);

                            Logger.WriteInformation(Logger.EventId.NotDefined, "MailProvider created: " + _instance.GetType().FullName);
                        }
                    }
                }
