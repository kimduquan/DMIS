...
namespace ContentRepository
{
    ...
    public sealed class RepositoryInstance : IDisposable
    {
        ...
        public void StartWorkflowEngine()
        {
            ...
            var t = TypeHandler.GetType("Workflow.InstanceManager");
            if (t != null)
            {
        ...
        private void LoadAssemblies()
        {
            ...
            else
            {
                ...
                asmNames = ContentRepository.Storage.TypeHandler.LoadAssembliesFrom(localBin);
            ...
            asmNames = ContentRepository.Storage.TypeHandler.LoadAssembliesFrom(pluginsPath);
        ...
        {
            object dummy;
            Communication.Messaging.IClusterChannel channel = null;

            try
            {
                ConsoleWrite("Initializing cache ... ");
                dummy = ContentRepository.DistributedApplication.Cache.Count;
                ConsoleWriteLine("ok.");

                ConsoleWrite("Starting message channel ... ");
                channel = ContentRepository.DistributedApplication.ClusterChannel;
                ConsoleWriteLine("ok.");
                ...
                dummy = ContentRepository.Schema.ContentType.GetByName("GenericContent");
                ConsoleWriteLine("ok.");
                ...
                IndexHealthMonitor.Start(_settings.Console);
                ...
                dummy = ResourceManager.Current;
                ConsoleWriteLine("ok.");

                BackgroundOperations.TaskManager.Start();
        ...
        List<ISnService> serviceInstances;
        ...
        private void RemoveIndexWriterLockFile()
        {
            ...
            if (System.IO.File.Exists(lockFilePath))
            {
                ...
                var endRetry = DateTime.UtcNow.AddSeconds(RepositoryConfiguration.LuceneLockDeleteRetryInterval);
        ...
        {
            Logger.WriteCritical(EventId.Error.UnhandledException, "Domain_UnhandledException", Logger.GetDefaultProperties, e.ExceptionObject);
        }
        ...
        public static void WaitForWriterLockFileIsReleased(WaitForLockFileType waitType)
        {
            ...
            if (!WaitForWriterLockFileIsReleased())
            {
                ...
                Logger.WriteError(Logger.EventId.NotDefined, string.Format(template,
        ...
        private static void SendWaitForLockErrorMail()
        {
            ...
            {
                Logger.WriteError(Logger.EventId.NotDefined, string.Format(WRITELOCKREMOVEEMAILERRORSTR,
