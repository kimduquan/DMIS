...
namespace ContentRepository.Tests
{
    ...
    public class FileTest : TestBase
    {
        ...
        [TestMethod]
        [Description("Reproduces a bug: lazy loaded binary property is RepositoryStream instead of BinaryDataValue")]
        public void File_LazyLoadedBinaryPropertyIsNotAStream()
        {
            var file = Node.Load<File>(CreateTestFile());
            var stream = file.Binary.GetStream();
            var buffer0 = new byte[stream.Length];
            stream.Read(buffer0, 0, buffer0.Length);

            var fileData = file.Data;
            var fileDataAcc = new PrivateObject(fileData);
            var dynamicData = (Dictionary<int, object>)fileDataAcc.GetField("dynamicData");
            var binaryPropType = ActiveSchema.PropertyTypes["Binary"];
            dynamicData.Remove(binaryPropType.Id);

            var originalFileName = file.Binary.FileName;
            var lazyLoadedValue = dynamicData[binaryPropType.Id];
            Assert.AreEqual(typeof(BinaryDataValue), lazyLoadedValue.GetType());
        }
