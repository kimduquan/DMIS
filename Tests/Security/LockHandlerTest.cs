...
namespace ContentRepository.Tests.Security
{
    ...
    public class LockHandlerTest : TestBase
	{
        ...
        public void LockHandler_UnlockAndVersionRaising()
        {
            ...
            folder.InheritableVersioningMode = ContentRepository.Versioning.InheritableVersioningType.MajorAndMinor;
            folder.Save();
        ...
        [TestMethod]
        public void LockHandler_TakeLockOver()
        {
            // create test content
            var content = Content.CreateNew("Car", Node.Load<Folder>(_rootNodePath), Guid.NewGuid().ToString());
            content.Save();

            // create test user
            var name = "LockHandler_TakeLockOver_User";
            var parentFolder = User.Administrator.Parent;
            var path = RepositoryPath.Combine(parentFolder.Path, name);
            var user = Node.LoadNode(path) as User;
            if (user == null)
            {
                user = new User(parentFolder) { Name = name };
            ...
            // grant permissions
            content.ContentHandler.Security.GetAclEditor()
                .SetPermission(user, true, PermissionType.Save, PermissionValue.Allow)
                .Apply();

            // locked by Admin
            content.CheckOut();
            Assert.AreEqual(User.Current.Id, content.ContentHandler.LockedById);

            // take over to test user
            Tools.TakeLockOver(content, user.Id.ToString());
            Assert.AreEqual(user.Id, content.ContentHandler.LockedById);

            // take over to me
            Tools.TakeLockOver(content, null);
            Assert.AreEqual(User.Current.Id, content.ContentHandler.LockedById);
		...
		private Page CreateTestPage()
		{
            ...
            Page f = new Page(Node.LoadNode(_rootNodePath)) { Name = Guid.NewGuid().ToString() };
