...
namespace ContentRepository.Tests.Security
{
	[TestClass()]
    public class PermissionTest : TestBase
	{
        ...
        {
            Repository.Root.Security.SetPermission(Group.Everyone, true, PermissionType.Open, PermissionValue.Allow);
        }
		...
		public void Permission_AllowDenyNotDefined()
		{
            ...
            alma.Security.SetPermission(user, true, PermissionType.RunApplication, PermissionValue.Allow);
            Assert.IsTrue(alma.Security.HasPermission((IUser)user, PermissionType.RunApplication), "#1");
			s = SecurityEntriesToString(alma);
			Assert.IsTrue(s.Contains(String.Concat("<Entry definedOn='", alma.Id, "' principalId='", user.Id, "'>")), "#2");
            Assert.IsTrue(s.Contains("<RunApplication value='Allow' />"), "#3");

            alma.Security.SetPermission(user, true, PermissionType.RunApplication, PermissionValue.Deny);
            Assert.IsFalse(alma.Security.HasPermission((IUser)user, PermissionType.RunApplication), "#4");
			s = SecurityEntriesToString(alma);
			Assert.IsTrue(s.Contains(String.Concat("<Entry definedOn='", alma.Id, "' principalId='", user.Id, "'>")), "#5");
            Assert.IsTrue(s.Contains("<RunApplication value='Deny' />"), "#6");

            alma.Security.SetPermission(user, true, PermissionType.RunApplication, PermissionValue.NonDefined);
            Assert.IsFalse(alma.Security.HasPermission((IUser)user, PermissionType.RunApplication), "#7");
...
//        }
        ...
        public void Security_SystemAccountAndOriginalUser()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(origUserId != -1, "Initial userId is -1");
                {
                    AccessProvider.ChangeToSystemAccount();
                    Assert.IsTrue(User.Current.Id == -1, "#1");
                    Assert.IsTrue(User.LoggedInUser.Id == origUserId, "#2");
                    {
                        AccessProvider.ChangeToSystemAccount();
                        Assert.IsTrue(User.Current.Id == -1, "#3");
                        Assert.IsTrue(User.LoggedInUser.Id == origUserId, "#4");
                        AccessProvider.RestoreOriginalUser();
                        Assert.IsTrue(User.Current.Id == -1, "#5");
                        Assert.IsTrue(User.LoggedInUser.Id == origUserId, "#6");
                    }
                    AccessProvider.RestoreOriginalUser();
        ...
        [TestMethod()]
        public void Permission_CustomPermission()
        {
            // create a brand new file
            string workingFolderPath = RepositoryPath.Combine(Repository.Root.Path, "WorkingFolder");
            Folder folder = CreateBrandNewFolder(workingFolderPath);

            // Deny Open, allow only See
            folder.Security.SetPermission(MolnarG, true, PermissionType.Custom01, PermissionValue.Allow);
            folder.Security.SetPermission(MolnarG, true, PermissionType.Custom02, PermissionValue.Deny);
            folder.Security.SetPermission(MolnarG, true, PermissionType.Custom03, PermissionValue.Allow);

            try
            {
                PermissionEvaluator.Reset();
                folder = Node.Load<Folder>(workingFolderPath);

                AccessProvider.Current.SetCurrentUser(MolnarG);

                Assert.IsTrue(folder.Security.HasPermission(PermissionType.Custom01, PermissionType.Custom01, PermissionType.Custom01, PermissionType.Custom01), "#1");
                Assert.IsTrue(folder.Security.GetPermission(PermissionType.Custom02) == PermissionValue.Deny, "#2");
            }
            finally
            {
                // restore
                ...
                folder.Security.SetPermission(MolnarG, true, PermissionType.Custom01, PermissionValue.NonDefined);
                folder.Security.SetPermission(MolnarG, true, PermissionType.Custom02, PermissionValue.NonDefined);
                folder.Security.SetPermission(MolnarG, true, PermissionType.Custom03, PermissionValue.NonDefined);
            }
        }

        [TestMethod]
        public void Permission_Copy_OnlyCustom()
        {
            var originalUser = User.Current;

            // create a brand new small tree
            var rootFolderPath = RepositoryPath.Combine(Repository.Root.Path, "WorkingFolder");
            var rootFolder = CreateBrandNewFolder(rootFolderPath);
            var sourceFolderPath = RepositoryPath.Combine(rootFolderPath, "SourceFolder");
            var sourceFolder = CreateBrandNewFolder(sourceFolderPath);
            var subFolderPath = RepositoryPath.Combine(sourceFolderPath, "SubFolder");
            var subFolder = CreateBrandNewFolder(subFolderPath);
            var targetFolderPath = RepositoryPath.Combine(rootFolderPath, "TargetFolder");
            var targetFolder = CreateBrandNewFolder(targetFolderPath);

            var targetSubFolderPath = targetFolderPath + "/SourceFolder/SubFolder";

            rootFolder.Security.SetPermission(Alex, true, PermissionType.Custom01, PermissionValue.Allow);
            rootFolder.Security.SetPermission(Alex, true, PermissionType.Custom02, PermissionValue.Deny);
            rootFolder.Security.SetPermission(MolnarG, true, PermissionType.Custom03, PermissionValue.Allow);
            rootFolder.Security.SetPermission(MolnarG, true, PermissionType.Custom04, PermissionValue.Deny);

            subFolder.Security.SetPermission(Alex, true, PermissionType.Custom05, PermissionValue.Allow);
            subFolder.Security.SetPermission(Alex, true, PermissionType.Custom06, PermissionValue.Deny);
            subFolder.Security.SetPermission(MolnarG, true, PermissionType.Custom07, PermissionValue.Allow);
            subFolder.Security.SetPermission(MolnarG, true, PermissionType.Custom08, PermissionValue.Deny);

            targetFolder.Security.SetPermission(Alex, true, PermissionType.Custom09, PermissionValue.Allow);
            targetFolder.Security.SetPermission(Alex, true, PermissionType.Custom10, PermissionValue.Deny);
            targetFolder.Security.SetPermission(MolnarG, true, PermissionType.Custom11, PermissionValue.Allow);
            targetFolder.Security.SetPermission(MolnarG, true, PermissionType.Custom12, PermissionValue.Deny);

            try
            {
                PermissionEvaluator.Reset();
                sourceFolder = Node.Load<Folder>(sourceFolderPath);
                targetFolder = Node.Load<Folder>(targetFolderPath);

                var rootEntries1 = string.Join(" | ", PermissionEvaluator.Instance.GetExplicitEntries(rootFolderPath).ToList());
                var subEntries1 = string.Join(" | ", PermissionEvaluator.Instance.GetExplicitEntries(subFolderPath).ToList());
                var targetEntries1 = string.Join(" | ", PermissionEvaluator.Instance.GetExplicitEntries(targetFolderPath).ToList());

                sourceFolder.CopyTo(targetFolder);
                var targetSubFolder = Node.Load<Folder>(targetSubFolderPath);

                var rootEntries2 = string.Join(" | ", PermissionEvaluator.Instance.GetExplicitEntries(rootFolderPath).ToList());
                var subEntries2 = string.Join(" | ", PermissionEvaluator.Instance.GetExplicitEntries(subFolderPath).ToList());
                var targetEntries2 = string.Join(" | ", PermissionEvaluator.Instance.GetExplicitEntries(targetFolderPath).ToList());
                var targetSubEntries = PermissionEvaluator.Instance.GetExplicitEntries(targetSubFolderPath);

                User.Current = Alex;
                Assert.AreEqual(PermissionValue.Allow, rootFolder.Security.GetPermission(PermissionType.Custom01));
                Assert.AreEqual(PermissionValue.Deny, rootFolder.Security.GetPermission(PermissionType.Custom02));
                Assert.AreEqual(PermissionValue.Allow, targetSubFolder.Security.GetPermission(PermissionType.Custom05));
                Assert.AreEqual(PermissionValue.Deny, targetSubFolder.Security.GetPermission(PermissionType.Custom06));
                User.Current = MolnarG;
                Assert.AreEqual(PermissionValue.Allow, rootFolder.Security.GetPermission(PermissionType.Custom03));
                Assert.AreEqual(PermissionValue.Deny, rootFolder.Security.GetPermission(PermissionType.Custom04));
                Assert.AreEqual(PermissionValue.Allow, targetSubFolder.Security.GetPermission(PermissionType.Custom07));
                Assert.AreEqual(PermissionValue.Deny, targetSubFolder.Security.GetPermission(PermissionType.Custom08));
            }
            finally
            {
        ...
        [TestMethod]
        public void Permission_Copy_NormalAndCustom()
        {
            var originalUser = User.Current;
            ...
            rootFolder.Security.SetPermission(Alex, true, PermissionType.RecallOldVersion, PermissionValue.Allow);
            rootFolder.Security.SetPermission(Alex, true, PermissionType.Custom01, PermissionValue.Allow);
            rootFolder.Security.SetPermission(Alex, true, PermissionType.Custom02, PermissionValue.Deny);
            subFolder.Security.SetPermission(Alex, true, PermissionType.DeleteOldVersion, PermissionValue.Allow);
            subFolder.Security.SetPermission(Alex, true, PermissionType.Custom03, PermissionValue.Allow);
            subFolder.Security.SetPermission(Alex, true, PermissionType.Custom04, PermissionValue.Deny);
            targetFolder.Security.SetPermission(Alex, true, PermissionType.SeePermissions, PermissionValue.Allow);
            targetFolder.Security.SetPermission(Alex, true, PermissionType.Custom05, PermissionValue.Allow);
            targetFolder.Security.SetPermission(Alex, true, PermissionType.Custom06, PermissionValue.Deny);

            ////---- permission setting with acl editor
            //rootFolder.Security.GetAclEditor()
            //    .SetPermission(Alex.Id, true, PermissionType.RecallOldVersion, PermissionValue.Allow)
            //    .SetPermission(Alex.Id, true, PermissionType.Custom01, PermissionValue.Allow)
            //    .SetPermission(Alex.Id, true, PermissionType.Custom02, PermissionValue.Deny)
            //    .Apply();
            //subFolder.Security.GetAclEditor()
            //    .SetPermission(Alex.Id, true, PermissionType.DeleteOldVersion, PermissionValue.Allow)
            //    .SetPermission(Alex.Id, true, PermissionType.Custom03, PermissionValue.Allow)
            //    .SetPermission(Alex.Id, true, PermissionType.Custom04, PermissionValue.Deny)
            //    .Apply();
            //targetFolder.Security.GetAclEditor()
            //    .SetPermission(Alex.Id, true, PermissionType.SeePermissions, PermissionValue.Allow)
            //    .SetPermission(Alex.Id, true, PermissionType.Custom05, PermissionValue.Allow)
            //    .SetPermission(Alex.Id, true, PermissionType.Custom06, PermissionValue.Deny)
            //    .Apply();
            ...
            try
            {
                ...
                User.Current = Alex;
                Assert.AreEqual(PermissionValue.Allow, targetSubFolder.Security.GetPermission(PermissionType.RecallOldVersion));
                Assert.AreEqual(PermissionValue.Allow, targetSubFolder.Security.GetPermission(PermissionType.DeleteOldVersion));
                Assert.AreEqual(PermissionValue.Allow, targetSubFolder.Security.GetPermission(PermissionType.SeePermissions));
                Assert.AreEqual(PermissionValue.Allow, targetSubFolder.Security.GetPermission(PermissionType.Custom01));
                Assert.AreEqual(PermissionValue.Deny, targetSubFolder.Security.GetPermission(PermissionType.Custom02));
                Assert.AreEqual(PermissionValue.Allow, targetSubFolder.Security.GetPermission(PermissionType.Custom03));
                Assert.AreEqual(PermissionValue.Deny, targetSubFolder.Security.GetPermission(PermissionType.Custom04));
                Assert.AreEqual(PermissionValue.Allow, targetSubFolder.Security.GetPermission(PermissionType.Custom05));
                Assert.AreEqual(PermissionValue.Deny, targetSubFolder.Security.GetPermission(PermissionType.Custom06));
            }
            finally
