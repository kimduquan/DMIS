...
{
    /// <summary>
    ///This is a test class for ContentRepository.Storage.Path and is intended
    ///to contain all ContentRepository.Storage.Path Unit Tests
    ///</summary>
    [TestClass()]
    public class PathTest : TestBase
    {
        ...
        [TestMethod()]
        [ExpectedException(typeof(RepositoryPathTooLongException))]
        public void Path_TooLong()
        {
            var content = Content.CreateNew("Folder", Repository.Root,
        ...
        public void Path_InvalidPathChar()
        {
            ...
            foreach (char c in chars)
                Assert.IsTrue(RepositoryPath.IsValidPath(String.Concat("/Root/a", c, "b")) == RepositoryPath.PathResult.InvalidPathChar, string.Concat("Not recognised invalid path char: ", c));
        }
        [TestMethod()]
        public void Path_ValidPathChar()
        {
            // try some characters that are valid (see the invalidcharpattern in the RepositoryPath class)
