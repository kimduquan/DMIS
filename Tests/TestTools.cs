...
namespace ContentRepository.Tests
{
	public static class TestTools
	{
		...
		{
			return (bool)new PrivateObject(new PrivateType("Storage", "ContentRepository.Storage.Data.DataProvider").InvokeStatic("get_Current")).Invoke("HasChild", nodeId);
		}
        ...
        public static PortalContext CreatePortalContext(string page, string query, TextWriter output = null)
        {
            var httpContext = CreateHttpContext(page, query, output);
            return PortalContext.Create(httpContext);
        }
        public static HttpContext CreateHttpContext(string page, string query, TextWriter output = null)
        {
            var ctx = CreateHttpContext(CreateRequest(page, query, output));
