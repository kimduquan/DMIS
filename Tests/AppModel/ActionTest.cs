...
namespace ContentRepository.Tests.AppModel
{
    /// <summary>
    ...
    public class ActionTest : TestBase
    {
        ...
        public static void CreateSandbox(TestContext testContext)
        {
            ...
            var siteAppsContentListOverrideApp6 = new Application(siteAppsFolderContentList);
                ...
                var siteAppsGenericContent = new SystemFolder(siteAppsFolder) { Name = "GenericContent" };
                siteAppsGenericContent.Save();
                //Root/TestRoot/(apps)/GenericContent/AppOvr
                var siteAppsGenericContentAppOvr = new Application(siteAppsGenericContent) { Name = "AppOvr", DisplayName = "ASDF", Icon = "ABC" };
                siteAppsGenericContentAppOvr.Save();

                //Root/TestRoot/(apps)/WebContent
                var siteAppsWebContent = new SystemFolder(siteAppsFolder) { Name = "WebContent" };
                siteAppsWebContent.Save();
                //Root/TestRoot/(apps)/WebContent/AppOvr
                var siteAppsWebContentAppOvr = new Application(siteAppsWebContent, "ApplicationOverride") { Name = "AppOvr", DisplayName = "QWER", Icon = "DEF" };
                siteAppsWebContentAppOvr.Save();

                //Root/TestRoot/(apps)/WebContentDemo
                var siteAppsWebContentDemo = new SystemFolder(siteAppsFolder) { Name = "WebContentDemo" };
                siteAppsWebContentDemo.Save();
                //Root/TestRoot/(apps)/WebContentDemo/AppOvr
                var siteAppsWebContentDemoAppOvr = new Application(siteAppsWebContentDemo, "ApplicationOverride") { Name = "AppOvr", DisplayName = "YXCV", Icon = "GHI" };
        ...
        [TestMethod]
        public void ApplicationOverride()
        {
            string oldDisplayName = null;
            string oldIcon = null;
            Application appWebContent = null;
            try
            {
                var siteSample1 = Content.Load("/Root/TestSiteForActionFramework/Sample");
                var genericcontent = Content.CreateNew("GenericContent", siteSample1.ContentHandler, null);
                genericcontent.Save();
                var webContent = Content.CreateNew("WebContent", siteSample1.ContentHandler, null);
                webContent.Save();
                var webContentDemo = Content.CreateNew("WebContentDemo", siteSample1.ContentHandler, null);
                webContentDemo.Save();

                var appGenericcontent = ApplicationStorage.Instance.GetApplication("AppOvr", genericcontent, null);
                appWebContent = ApplicationStorage.Instance.GetApplication("AppOvr", webContent, null);
                var appWebContentDemo = ApplicationStorage.Instance.GetApplication("AppOvr", webContentDemo, null);

                Assert.IsTrue(appGenericcontent.DisplayName == "ASDF", String.Format("appGenericcontent.DisplayName is {0}, expected: ASDF", appGenericcontent.DisplayName));
                Assert.IsTrue(appWebContent.DisplayName == "QWER", String.Format("appWebContent.DisplayName is {0}, expected: QWER", appWebContent.DisplayName));
                Assert.IsTrue(appWebContentDemo.DisplayName == "YXCV", String.Format("appWebContentDemo.DisplayName is {0}, expected: YXCV", appWebContentDemo.DisplayName));

                Assert.IsTrue(appGenericcontent.Icon == "ABC", String.Format("appGenericcontent.Icon is {0}, expected: ABC", appGenericcontent.Icon));
                Assert.IsTrue(appWebContent.Icon == "DEF", String.Format("appWebContent.Icon is {0}, expected: DEF", appWebContent.Icon));
                Assert.IsTrue(appWebContentDemo.Icon == "GHI", String.Format("appWebContentDemo.Icon is {0}, expected: GHI", appWebContentDemo.Icon));

                //---------------------------------------------

                oldDisplayName = appWebContent.Override.DisplayName;
                oldIcon = appWebContent.Override.Icon;
                appWebContent.Override.DisplayName = null;
                appWebContent.Override.Icon = null;
                appWebContent.Override.Save();

                appGenericcontent = ApplicationStorage.Instance.GetApplication("AppOvr", genericcontent, null);
                appWebContent = ApplicationStorage.Instance.GetApplication("AppOvr", webContent, null);
                appWebContentDemo = ApplicationStorage.Instance.GetApplication("AppOvr", webContentDemo, null);

                Assert.IsTrue(appGenericcontent.DisplayName == "ASDF", String.Format("appGenericcontent.DisplayName is {0}, expected: ASDF", appGenericcontent.DisplayName));
                Assert.IsTrue(appWebContent.DisplayName == "ASDF", String.Format("appWebContent.DisplayName is {0}, expected: ASDF", appWebContent.DisplayName));
                Assert.IsTrue(appWebContentDemo.DisplayName == "YXCV", String.Format("appWebContentDemo.DisplayName is {0}, expected: YXCV", appWebContentDemo.DisplayName));

                Assert.IsTrue(appGenericcontent.Icon == "ABC", String.Format("appGenericcontent.Icon is {0}, expected: ABC", appGenericcontent.Icon));
                Assert.IsTrue(appWebContent.Icon == "ABC", String.Format("appWebContent.Icon is {0}, expected: ABC", appWebContent.Icon));
                Assert.IsTrue(appWebContentDemo.Icon == "GHI", String.Format("appWebContentDemo.Icon is {0}, expected: GHI", appWebContentDemo.Icon));
            }
            finally
            {
                appWebContent.Override.DisplayName = oldDisplayName;
                appWebContent.Override.Icon = oldIcon;
            }
        }
    }
}
