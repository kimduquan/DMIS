...
";
        ...
        public void Delete_WithDelete_WithoutDeletePermissions()
        {
            ...
            Assert.IsTrue(exceptionThrown, "Security exception should be thrown.");
            CheckNumberOfItemsInTrash(0);
        }
        ...
        [TestMethod]
        public void Delete_WithDelete_CreatorPermissions()
        {
            var sampleContentId = EnsureNode("[TestRoot]/SampleContent");
            var sampleContent = Content.Load(sampleContentId);
            var creatorId = sampleContent.ContentHandler.CreatedById;

            // make sure that there is a permission entry for the Creators group on the original content
            sampleContent.Security.SetPermission(Group.Creators, true, PermissionType.Open, PermissionValue.Allow);

            // make sure that there is no permission entry for the creator on the original content
            Assert.IsFalse(sampleContent.Security.GetEffectiveEntries().Any(s => s.PrincipalId == creatorId), "There is an explicite entry for the creator on the original content. This ruins the test.");

            DeleteWithDeleteNode("[TestRoot]/SampleContent");

            sampleContent = Content.Load(sampleContentId);
            var trashBag = sampleContent.ContentHandler.Parent;

            // the trashbag must contain a new explicite permission entry for the creator of the original document
            Assert.IsTrue(trashBag.Security.GetExplicitEntries().Any(s => s.PrincipalId == creatorId));
        }
