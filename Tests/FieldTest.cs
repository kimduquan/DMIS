...
using SnControls = Portal.UI.Controls;
using System.Text;
...
<ContentType name='FieldOnGenericTest' parentType='GenericContent' handler='ContentRepository.GenericContent' xmlns='http://schemas.com/ContentRepository/ContentTypeDefinition'>
	...
	<Fields>
        ...
        <Field name='Type' type='NodeType'>
          ...
          <Indexing>
            <IndexHandler>Search.Indexing.ExclusiveTypeIndexHandler</IndexHandler>
          </Indexing>
          <Configuration>
            <Visible>false</Visible>
            <ReadOnly>true</ReadOnly>
        ...
        <Field name='TypeIs' type='NodeType'>
          ...
          <Bind property='NodeType' />
          <Indexing>
            <IndexHandler>Search.Indexing.TypeTreeIndexHandler</IndexHandler>
          </Indexing>
          <Configuration>
            <Visible>false</Visible>
            <ReadOnly>true</ReadOnly>
        ...
        <Field name='InTree' type='ShortText'>
          ...
          <Indexing>
            <Analyzer>Lucene.Net.Analysis.KeywordAnalyzer</Analyzer>
            <IndexHandler>Search.Indexing.InTreeIndexHandler</IndexHandler>
          </Indexing>
          <Configuration>
            <Visible>false</Visible>
            <ReadOnly>true</ReadOnly>
        ...
        <Field name='InFolder' type='ShortText'>
          ...
          <Indexing>
            <Analyzer>Lucene.Net.Analysis.KeywordAnalyzer</Analyzer>
            <IndexHandler>Search.Indexing.InFolderIndexHandler</IndexHandler>
          </Indexing>
          <Configuration>
            <Visible>false</Visible>
            <ReadOnly>true</ReadOnly>
...
<ContentType name='FieldOnHandlerTest' parentType='GenericContent' handler='ContentRepository.Tests.ContentHandlers.FieldTestHandler' 
		xmlns='http://schemas.com/ContentRepository/ContentTypeDefinition'
		xmlns:q='http://schemas.com/ContentRepository/SearchExpression'>
	<DisplayName>FieldTest</DisplayName>
	<Description>Test type for ContentHandler and Field connection tests</Description>
	<Fields>
		<Field name='ShortText'  type='ShortText'></Field>
...
<ContentType name='DefaultValueTest' parentType='FieldOnHandlerTest' handler='ContentRepository.Tests.ContentHandlers.DefaultValueTest' 
        xmlns='http://schemas.com/ContentRepository/ContentTypeDefinition'
        xmlns:q='http://schemas.com/ContentRepository/SearchExpression'>
    <DisplayName>FieldTest</DisplayName>
    <Description>Test type for ContentHandler and Field connection tests</Description>
    <Fields>
        ...
        public void GenericControlByField()
        {
            ...
            Assert.IsTrue(CheckTypes(typeof(ColorField), typeof(SnControls.ColorPicker), out message), message);
        ...
        public void XssProtection()
        {
            ...
            Assert.IsTrue(dataDefault == dataHtml, "LongText's default output is not TEXT");
        ...
        //    //field = fieldPrototypes["ContentRepository.Fields.ShortTextField"].CreateInstance();
        //    stopper.Stop();
        ...
        //    stopper = Stopwatch.StartNew();
        //    for (int i = 0; i < count; i++)
        //        field = fieldManager1.CreateField("ContentRepository.Fields.ShortTextField");
        //    stopper.Stop();
        ...
        //    stopper = Stopwatch.StartNew();
        //    for (int i = 0; i < count; i++)
        //        field = fieldManager2.CreateField("ContentRepository.Fields.ShortTextField");
        //    stopper.Stop();
        ...
        public void Field_NamingConvention_Correct()
        {
            ...
            string ctd = @"<?xml version='1.0' encoding='utf-8'?>
				<ContentType name='FieldNamingTest' parentType='GenericContent' handler='ContentRepository.GenericContent' xmlns='http://schemas.com/ContentRepository/ContentTypeDefinition'>
        ...
        public void Field_NamingConvention_BadContentListField()
        {
            ...
            string ltd = @"<?xml version='1.0' encoding='utf-8'?>
				<ContentListDefinition name='FieldNamingTest' xmlns='http://schemas.com/ContentRepository/ContentTypeDefinition'>
					<Fields><ContentListField name='ContentListField1' type='ShortText' /></Fields></ContentListDefinition>";
        ...
        public void FieldOnHandler_NumberOnInt64()
        {
            ...
            Assert.IsTrue(originalValue == testIntValue, "#6");
            currentValue = (decimal)content[fieldName];
            Assert.IsTrue(currentValue == testHandlerValue, "#7");
            handlerValue = handler.Int64;
            Assert.IsTrue(handlerValue == testHandlerValue, "#8");
        ...
        [TestMethod]
        public void FieldOnHandler_Ref_MultipleDistinct()
        {
            var axis = "GeneralReference";

            var node = new GenericContent(_testRoot, "FieldOnHandlerTest") { Name = Guid.NewGuid().ToString() };
            node.AddReference(axis, User.Administrator);
            node.AddReference(axis, User.Visitor);
            node.AddReference(axis, User.Administrator);
            node.AddReference(axis, User.Administrator);
            node.AddReference(axis, User.Visitor);
            node.Save();

            var node1 = Node.LoadNode(node.Id);
            var actual = String.Join(", ", node1.GetReferences(axis).Select(x => x.Id).OrderBy(x => x));
            var expected = String.Concat(User.Administrator.Id, ", ", User.Visitor.Id);
        ...
        private ContentAccessor WhoAndWhenTest(string contentTypeName)
        {
            ...
            //Assert.IsTrue(originalValue.Who == defaultUserValue && ((TimeSpan)(DateTime.UtcNow - originalValue.When)).TotalMinutes < 1, contentTypeName + "####1 Storage2: DateTime default value");
            ...
            currentUserValue = currrenValue.Who;
            currentDateValue = currrenValue.When;
            Assert.IsTrue(currentUserValue.Id == testUserValue.Id && currentDateValue == testHandlerDateValue, contentTypeName + "#7");
            handlerUserValue = (NodeList<Node>)content.ContentHandler[userPropertyName];
            handlerDateValue = (DateTime)content.ContentHandler[datePropertyName];
            ...
            defaultValue.When = DateTime.UtcNow;
            content[fieldName] = defaultValue;
            originalValue = (WhoAndWhenField.WhoAndWhenData)content.Fields[fieldName].OriginalValue;
            originalUserValue = originalValue.Who;
            originalDateValue = originalValue.When;
        ...
        public void Field_OuterField_OnTheContent()
        {
            ContentTypeInstaller.InstallContentType(@"<?xml version='1.0' encoding='utf-8'?>
                <ContentType name='OuterFieldTestContentType' parentType='GenericContent' handler='ContentRepository.GenericContent' xmlns='http://schemas.com/ContentRepository/ContentTypeDefinition'>
                    <Fields>
