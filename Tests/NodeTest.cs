...
using ContentRepository.Storage.Data.SqlClient;
using Search;
using ContentRepository.Tests.Storage;
...
<ContentType name=""Folder"" parentType=""GenericContent"" handler=""ContentRepository.Folder"" xmlns=""http://schemas.com/ContentRepository/ContentTypeDefinition"">
	...
	<Fields>
        ...
        public void Storage2_Milestone4_UseReferences()
        {
            ...
            if (ContentType.GetByName("File2") == null)
                ContentTypeInstaller.InstallContentType(@"<?xml version=""1.0"" encoding=""utf-8""?>
					<ContentType name=""File2"" parentType=""File"" handler=""ContentRepository.File"" xmlns=""http://schemas.com/ContentRepository/ContentTypeDefinition"">
						<DisplayName>Folder</DisplayName>
						<Description>Use folders to group information to one place</Description>
						<Icon>Folder</Icon>
						<Fields>
        ...
        public void Bug_ImmediatelyDeletedReference()
        {
            if (ContentType.GetByName("ImmediatelyDeletedReferenceTest") == null)
                ContentTypeInstaller.InstallContentType(@"<?xml version=""1.0"" encoding=""utf-8""?>
					<ContentType name=""ImmediatelyDeletedReferenceTest"" parentType=""GenericContent"" handler=""ContentRepository.GenericContent"" xmlns=""http://schemas.com/ContentRepository/ContentTypeDefinition"">
            ...
            try
            {
                ...
                // The INSERT statement conflicted with the FOREIGN KEY constraint "FK_ReferenceProperties_Nodes". The conflict occurred in database "ContentRepository", table "dbo.Nodes", column 'NodeId'. The statement has been terminated.
                alice.Save();
            }
            catch (Exception exc)
            {
        ...
        public void Storage2_Bug_BoolQuery()
        {
            if (ContentType.GetByName("BoolTest") == null)
                ContentTypeInstaller.InstallContentType(@"<?xml version=""1.0"" encoding=""utf-8""?>
					<ContentType name=""BoolTest"" parentType=""GenericContent"" handler=""ContentRepository.GenericContent"" xmlns=""http://schemas.com/ContentRepository/ContentTypeDefinition"">
						<DisplayName>BoolTest</DisplayName>
						<Fields>
        ...
        public void Node_SetDates()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < users.Length; i++)
                    users[i] = ContentRepository.Tests.Security.PermissionTest.LoadOrCreateUser(Guid.NewGuid().ToString(), Guid.NewGuid().ToString(), userFolder);
    ...
    public class NodeTest : TestBase
	{
        ...
        [TestMethod]
        public void MultistepSaving_QueryBySavingState()
        {
            var initialCeatingResult = ContentQuery.Query(String.Format("SavingState:${0} .COUNTONLY .AUTOFILTERS:OFF .LIFESPAN:OFF", (int)ContentSavingState.Creating));
            var initialCreatingCount = initialCeatingResult.Count;
            var initialModifyingResult = ContentQuery.Query(String.Format("SavingState:${0} .COUNTONLY .AUTOFILTERS:OFF .LIFESPAN:OFF", (int)ContentSavingState.Modifying));
            var initialmodifyingCount = initialModifyingResult.Count;
            ...
            var creatingResult = ContentQuery.Query(String.Format("SavingState:'${0}' .COUNTONLY .AUTOFILTERS:OFF .LIFESPAN:OFF", (int)ContentSavingState.Creating));
            Assert.AreEqual(3, creatingResult.Count - initialCreatingCount);
            var modifyingResult = ContentQuery.Query(String.Format("SavingState:${0} .COUNTONLY .AUTOFILTERS:OFF .LIFESPAN:OFF", (int)ContentSavingState.Modifying));
            Assert.AreEqual(2, modifyingResult.Count - initialmodifyingCount);
        }
        ...
        public void Cache_NodeHead_Folder()
        {
            using (new CacheContentAfterSaveModeHacker(RepositoryConfiguration.CacheContentAfterSaveOption.Containers))
            {
                ...
                newFolder.DisableObserver(typeof(Portal.Workspaces.JournalObserver));
                newFolder.DisableObserver(typeof(ApplicationModel.AppStorageInvalidator));
                newFolder.DisableObserver(typeof(Preview.DocumentPreviewObserver));
                newFolder.DisableObserver(typeof(ContentRepository.Tests.Storage.LoggedTestObserver));
                newFolder.DisableObserver(typeof(ContentRepository.Storage.AppModel.AppCacheInvalidator));
                newFolder.DisableObserver(typeof(ContentRepository.Storage.AppModel.RepositoryEventRouter));
                newFolder.DisableObserver(typeof(Workflow.WorkflowNotificationObserver));
                newFolder.DisableObserver(typeof(Messaging.NotificationObserver));
                newFolder.Save();
                var id = newFolder.Id;
                ...
                folder.DisableObserver(typeof(Portal.Workspaces.JournalObserver));
                folder.DisableObserver(typeof(ApplicationModel.AppStorageInvalidator));
                folder.DisableObserver(typeof(Preview.DocumentPreviewObserver));
                folder.DisableObserver(typeof(ContentRepository.Tests.Storage.LoggedTestObserver));
                folder.DisableObserver(typeof(ContentRepository.Storage.AppModel.AppCacheInvalidator));
                folder.DisableObserver(typeof(ContentRepository.Storage.AppModel.RepositoryEventRouter));
                folder.DisableObserver(typeof(Workflow.WorkflowNotificationObserver));
                folder.DisableObserver(typeof(Messaging.NotificationObserver));
                folder.Save();
        ...
        public void Cache_NodeHead_File()
        {
            ...
            using (new CacheContentAfterSaveModeHacker(RepositoryConfiguration.CacheContentAfterSaveOption.Containers))
            {
                ...
                newFile.DisableObserver(typeof(Portal.Workspaces.JournalObserver));
                newFile.DisableObserver(typeof(ApplicationModel.AppStorageInvalidator));
                newFile.DisableObserver(typeof(Preview.DocumentPreviewObserver));
                newFile.DisableObserver(typeof(ContentRepository.Tests.Storage.LoggedTestObserver));
                newFile.DisableObserver(typeof(ContentRepository.Storage.AppModel.AppCacheInvalidator));
                newFile.DisableObserver(typeof(ContentRepository.Storage.AppModel.RepositoryEventRouter));
                newFile.DisableObserver(typeof(Workflow.WorkflowNotificationObserver));
                newFile.DisableObserver(typeof(Messaging.NotificationObserver));
                newFile.Save();
                var id = newFile.Id;
                ...
                file.DisableObserver(typeof(Portal.Workspaces.JournalObserver));
                file.DisableObserver(typeof(ApplicationModel.AppStorageInvalidator));
                file.DisableObserver(typeof(Preview.DocumentPreviewObserver));
                file.DisableObserver(typeof(ContentRepository.Tests.Storage.LoggedTestObserver));
                file.DisableObserver(typeof(ContentRepository.Storage.AppModel.AppCacheInvalidator));
                file.DisableObserver(typeof(ContentRepository.Storage.AppModel.RepositoryEventRouter));
                file.DisableObserver(typeof(Workflow.WorkflowNotificationObserver));
                file.DisableObserver(typeof(Messaging.NotificationObserver));
                file.Save();
        ...
        public void Node_Load_Bug5527()
        {
            ...
            try
            {
                ContentTypeInstaller.InstallContentType(
                    @"<ContentType name='Car_Bug5527' parentType='Car' handler='ContentRepository.GenericContent' xmlns='http://schemas.com/ContentRepository/ContentTypeDefinition'>
                    <Fields>
