    ...
    public class EvaluatorTest : TestBase
    {
        ...
        public void DefaultValue_Scripted_Simple()
        {
            var result = DefaultValueTest(new string[][]
            {
                ...
                new string[]{"DateTimeField", "[Script:jScript]DateTime.UtcNow[/Script]"},
            ...
            Assert.IsTrue(DateTime.UtcNow - (DateTime)result[1] < new TimeSpan(0, 0, 2));
            ...
            //Assert.IsTrue(((Node)result[5]).Path == User.Administrator.Path);
            Assert.IsTrue((string)result[5] == "Default short text value");
        }

        #region CTD
        string ctdFormat = @"<?xml version='1.0' encoding='utf-8'?>
<ContentType name='DefaultValueTesterContentType' parentType='GenericContent' handler='ContentRepository.GenericContent' xmlns='http://schemas.com/ContentRepository/ContentTypeDefinition'>
    <Fields>
