...
using ContentRepository.Search.Indexing;
using Search.Parser;
    ...
    public class IndexingTests : TestBase
    {
        ...
        private class ColorWithNameIndexHandler : FieldIndexHandler, IIndexValueConverter<Color>, IIndexValueConverter
        {
            public override IEnumerable<IndexFieldInfo> GetIndexFieldInfos(Field snField, out string textExtract)
            {
                ...
                var colorString = ContentRepository.Fields.ColorField.ColorToString(color).ToLower();
            ...
            {
                return ContentRepository.Fields.ColorField.ColorFromString(lucFieldValue);
            }
        ...
        public void Indexing_RecoverStoredField()
        {
            var ctdTemplate = @"<?xml version='1.0' encoding='utf-8'?>
                <ContentType name='{0}' parentType='GenericContent' handler='ContentRepository.GenericContent' xmlns='http://schemas.com/ContentRepository/ContentTypeDefinition'>
                  <Fields>
            ...
            var query = LucQuery.Parse("BodyColorIndexingTests:Red .AUTOFILTERS:OFF");
            try
            {
                ...
                Assert.IsTrue(ContentRepository.Fields.ColorField.ColorToString(fieldValueColor) == ContentRepository.Fields.ColorField.ColorToString(Color.Red), "Colors are not equal.");
            }
            finally
            {
                if (content != null)
        ...
        public void Indexing_Disallow()
        {
            ...
            ContentTypeInstaller.InstallContentType(
                @"<ContentType name=""NotIndexedCar"" parentType=""Car"" handler=""ContentRepository.GenericContent"" xmlns=""http://schemas.com/ContentRepository/ContentTypeDefinition"">
                    <AllowIndexing>false</AllowIndexing><Fields><Field name=""SpecialData"" type=""ShortText"" /></Fields></ContentType>",
                @"<ContentType name=""NotIndexedSystemFolder"" parentType=""SystemFolder"" handler=""ContentRepository.GenericContent"" xmlns=""http://schemas.com/ContentRepository/ContentTypeDefinition"">
            ...
            var root = new SystemFolder(TestRoot) { Name = "Indexing_RecoverStoredField" };
            root.Save();
        ...
        public void Indexing_AddTextExtract()
        {
            ...
            var node = ContentQuery.Query(word, new QuerySettings { EnableAutofilters = FilterStatus.Disabled }).Nodes.FirstOrDefault();
        ...
        public void Querying_SortingWithCultureInfo()
        {
            ...
            try
            {
                ...
                var cqresult = ContentQuery.Query("TypeIs:Car AND Name:CarSorting* .SORT:Make .AUTOFILTERS:OFF");
        ...
        public void Querying_ParserAndKeywordAnalyzer()
        {
            ...
            {
                var count = ContentQuery.Query(query.Item1 + " .AUTOFILTERS:OFF .COUNTONLY").Count;
        ...
        [TestMethod]
        public void Querying_ParseCountOnlyCommentedOut()
        {
            var count = ContentQuery.Query("Keywords:bye/* .COUNTONLY").Count;
            // expected: no error
        }
