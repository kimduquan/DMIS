...
namespace ContentRepository.Tests
{
    ...
    public class LinqTests : TestBase
    {
        ...
        public void Linq_NullString()
        {
            ...
            var x = Search.ContentQuery.Query("Model:'' .AUTOFILTERS:OFF").Count;
        ...
        public void Linq_Bool_Negation()
        {
            ...
            q = GetQueryString(Content.All.Where(c => !((Portal.Site)c.ContentHandler).EnableClientBasedCulture));
        ...
        public void Linq_OfType()
        {
            Assert.AreEqual("+TypeIs:site +EnableClientBasedCulture:yes",
                GetQueryString(Content.All.OfType<Portal.Site>().Where(c => c.EnableClientBasedCulture == true)));
        }
...
Id:<6 .AUTOFILTERS:OFF";
        ...
        {
            // There was a bug in a customer code that caused StackOverflowException but in this case has never been reproduced.
