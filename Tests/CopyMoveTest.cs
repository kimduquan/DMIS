...
";
        ...
        {
            var docs = Search.Indexing.LuceneManager.GetDocumentsByNodeId(nodeId);
            return String.Join(", ", docs.Select(d => d.Get(LucObject.FieldName.Version)).ToArray());
        }

        [TestMethod]
        public void Move_WithAspect()
        {
            // create an aspect
            var aspect1 = AspectTests.EnsureAspect("CopyMoveTest_Move_WithAspect");
        ...
        [TestMethod]
        public void Copy_AfterCopy_BinaryIdInUnderlyingStream()
        {
            var sourceText = new StringBuilder().Insert(0, "Source file content. ", 100).ToString();
            var targetText = "Target file content";

            EnsureNode("[TestRoot]/SourceFolder");
            EnsureNode("[TestRoot]/TargetFolder");
            var srcFolder = LoadNode("[TestRoot]/SourceFolder");
            var srcFile = new File(srcFolder);
            srcFile.Name = "MyFile1";
            srcFile.Binary.SetStream(Tools.GetStreamFromString(sourceText));
            srcFile.Binary.FileName = "1.txt";
            srcFile.Save();
            var srcBinaryIdBefore = srcFile.Binary.Id;

            CopyNode("[TestRoot]/SourceFolder/MyFile1", "[TestRoot]/TargetFolder");

            var srcBinaryIdAfter = srcFile.Binary.Id;

            var targetfile = (File)LoadNode("[TestRoot]/TargetFolder/MyFile1");
            var targetBinaryId1 = targetfile.Binary.Id;
            var targetBinaryIdFromStream1 = GetBinaryPropertyIdOfUnderlyingStream(targetfile.Binary);

            Assert.IsTrue(srcBinaryIdBefore == srcBinaryIdAfter, "srcBinaryIdBefore and srcBinaryIdAfter are not equal.");
            Assert.IsTrue(srcBinaryIdBefore != targetBinaryId1, "srcBinaryIdBefore and targetBinaryId1 are equal.");
            Assert.IsTrue(targetBinaryId1 == targetBinaryIdFromStream1, "targetBinaryId1 and targetBinaryId2 are not equal.");

            targetfile.Binary.SetStream(Tools.GetStreamFromString(targetText));
            targetfile.Save();

            var targetBinaryId2 = targetfile.Binary.Id;
            var targetBinaryIdFromStream2 = GetBinaryPropertyIdOfUnderlyingStream(targetfile.Binary);

            Assert.IsTrue(targetBinaryId1 == targetBinaryId2, "targetBinaryId1 and targetBinaryId2 are equal.");
            Assert.IsTrue(targetBinaryId2 == targetBinaryIdFromStream2, "targetBinaryId1 and targetBinaryId2 are not equal.");
            Assert.IsTrue(targetBinaryIdFromStream1 == targetBinaryIdFromStream2, "targetBinaryIdFromStream1 and targetBinaryId2 are not equal.");
        }
        private int GetBinaryPropertyIdOfUnderlyingStream(BinaryData binaryData)
        {
            var binaryDataAcc = new PrivateObject(binaryData);
            var rawData = binaryDataAcc.GetProperty("RawData");

            var rawDataAcc = new PrivateObject(rawData);
            var stream = rawDataAcc.GetProperty("Stream");

            var repoStream = stream as RepositoryStream;
            if (repoStream != null)
                return repoStream.BinaryPropertyId;

            repoStream = binaryData.GetStream() as RepositoryStream;
            if (repoStream != null)
                return repoStream.BinaryPropertyId;
        ...
        [TestMethod]
        public void Copy_SystemSubstructure()
        {
            IUser originalUser = AccessProvider.Current.GetCurrentUser();
            try
            {
                IUser visitor = Node.LoadNode(RepositoryConfiguration.VisitorUserId) as IUser;
                EnsureNode("[TestRoot]/Source");
                EnsureNode("[TestRoot]/Source/SystemFolder1");
                EnsureNode("[TestRoot]/Source/SystemFolder1/Item1");
                EnsureNode("[TestRoot]/Source/SystemFolder1/Item2");
                EnsureNode("[TestRoot]/Source/SystemFolder2");
                EnsureNode("[TestRoot]/Source/SystemFolder2/Item1");
                EnsureNode("[TestRoot]/Source/SystemFolder2/Item2");
                EnsureNode("[TestRoot]/Target");
                Node sourceNode = LoadNode("[TestRoot]/Source");
                Node targetNode = LoadNode("[TestRoot]/Target");
                sourceNode.Security.SetPermission(visitor, true, PermissionType.Open, PermissionValue.Allow);
            ...
            Assert.IsTrue(Node.Exists(DecodePath("[TestRoot]/Target/Source")));
            Assert.IsTrue(Node.Exists(DecodePath("[TestRoot]/Target/Source/SystemFolder1")));
            Assert.IsTrue(Node.Exists(DecodePath("[TestRoot]/Target/Source/SystemFolder1/Item1")));
            Assert.IsTrue(Node.Exists(DecodePath("[TestRoot]/Target/Source/SystemFolder1/Item2")));
            Assert.IsTrue(Node.Exists(DecodePath("[TestRoot]/Target/Source/SystemFolder2")));
            Assert.IsTrue(Node.Exists(DecodePath("[TestRoot]/Target/Source/SystemFolder2/Item1")));
            Assert.IsTrue(Node.Exists(DecodePath("[TestRoot]/Target/Source/SystemFolder2/Item2")));
        }
        ...
        [TestMethod]
        public void Copy_ContentList_FIX_SwappedBindings()
        {
            EnsureNode("[TestRoot]/SourceFolder/ContentList/SourceContentListItem");
            EnsureNode("[TestRoot]/TargetFolder");

            //-- swap binding of field 1 and 3.
            var list = (ContentList)LoadNode("[TestRoot]/SourceFolder/ContentList");
            var bindings = list.ContentListBindings;
            var temp = new Dictionary<string, List<string>>
            {
                { "#ContentListField1", bindings["#ContentListField3"] },
                { "#ContentListField2", bindings["#ContentListField2"] },
                { "#ContentListField3", bindings["#ContentListField1"] },
            };
            list.ContentListBindings = temp;
            list.Save();

            //-- save a content with list field values
            var node0 = LoadNode("[TestRoot]/SourceFolder/ContentList/SourceContentListItem");
            node0["#String_0"] = "value0";
            node0["#String_1"] = "value1";
            node0.Save();
            var sourceItemContent = Content.Create(node0);

            //-- copy a folder containing the list
            CopyNode("[TestRoot]/SourceFolder", "[TestRoot]/TargetFolder");

            //-- properties are changed (this is the base of the issue)
            var node1 = LoadNode("[TestRoot]/TargetFolder/SourceFolder/ContentList/SourceContentListItem");
            Assert.AreEqual("value1", node1["#String_0"]);
            Assert.AreEqual("value0", node1["#String_1"]);

            //-- field values are not changed
            var targetItemContent = Content.Create(node1);
            Assert.AreEqual((string)sourceItemContent["#ContentListField1"], (string)targetItemContent["#ContentListField1"]);
            Assert.AreEqual((string)sourceItemContent["#ContentListField3"], (string)targetItemContent["#ContentListField3"]);
        }
        ...
        public void Rename_SubtreeCheck()
        {
            ...
            var q1 = new ContentRepository.Storage.Search.NodeQuery(
                new ContentRepository.Storage.Search.StringExpression(
                    ContentRepository.Storage.Search.StringAttribute.Path,
                    ContentRepository.Storage.Search.StringOperator.StartsWith,
                    String.Concat(_testRootPath, "/AA")));
            var result1 = q1.Execute();
            Assert.IsTrue(result1.Count == 0, "#1");

            var q2 = new ContentRepository.Storage.Search.NodeQuery(
            new ContentRepository.Storage.Search.StringExpression(
                ContentRepository.Storage.Search.StringAttribute.Path,
                ContentRepository.Storage.Search.StringOperator.StartsWith,
        ...
        public void Move_Concurrency()
        {
            ...
            sourceNode1.Name = Guid.NewGuid().ToString(); // rename to avoid name conflict
            sourceNode1.Save();
            try
            {
                sourceNode2.MoveTo(targetNode);
            }
            catch (NodeIsOutOfDateException e) //not logged, not rethrown
            {
                thrown = true;
            }
        ...
        [TestMethod]
        public void Copy_Concurrency()
        {
            var source = Content.CreateNew("Car", TestRoot, null);
            source.Save();
            var sourcePath = source.Path;

            var target1 = Content.CreateNew("SystemFolder", TestRoot, Guid.NewGuid().ToString());
            target1.Save();
            var target1Path = target1.Path;

            var target2 = Content.CreateNew("SystemFolder", TestRoot, Guid.NewGuid().ToString());
            target2.Save();
            var target2Path = target2.Path;

            var sourceNode1 = Node.LoadNode(sourcePath);
            var sourceNode2 = Node.LoadNode(sourcePath);
            var targetNode1 = Node.LoadNode(target1Path);
            var targetNode2 = Node.LoadNode(target2Path);

            var thrown = false;
            sourceNode1.MoveTo(targetNode1);
            try
            {
                sourceNode2.CopyTo(targetNode2);
            }
            catch (ContentNotFoundException e) //not logged, not rethrown
            {
                thrown = true;
            }

            Assert.IsTrue(thrown, "InvalidOperationException is not thrown");
        }
