		...
		private static AppDomain[] Domains
		{
			get
			{
				if (__domains == null)
				{
                    ...
                    var configPath = Path.Combine(baseDir, "tests.dll.config");
                ...
                if (__remotedEngines == null)
                {
                    lock (_remotedEnginesLock)
                    {
                        if (__remotedEngines == null)
                        {
                            ...
                            for (int i = 0; i < _remotedEngineCount; i++)
                            {
                                remotedEngines[i] = (IRemotedTests)Domains[i].CreateInstanceAndUnwrap("Tests", "ContentRepository.Tests.CrossDomain.RemotedTests");
