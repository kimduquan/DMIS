...
namespace ContentRepository.Tests
{
    ...
    public class GlobalizationTests : TestBase
    {
        ...
        public void Globalization_ResKeyParser()
        {
            ...
            var ok = ResourceManager.ParseResourceKey(src = "$Resources:ClassName,Key", out className, out name);
            ...
            ok = ResourceManager.ParseResourceKey(src = "$   Resources:   ClassName  ,  Key   ", out className, out name);
            Assert.IsTrue(ok, "Failed to parse: " + src);
            Assert.IsTrue(className == "ClassName", String.Format("ClassName is '{0}', expected: 'ClassName', src: {1}", className, src));
            Assert.IsTrue(name == "Key", String.Format("Name is '{0}', expected: 'Key', src: {1}", name, src));

            ok = ResourceManager.ParseResourceKey(src = "$ClassName,Key", out className, out name);
            Assert.IsTrue(ok, "Failed to parse: " + src);
            Assert.IsTrue(className == "ClassName", String.Format("ClassName is '{0}', expected: 'ClassName', src: {1}", className, src));
            Assert.IsTrue(name == "Key", String.Format("Name is '{0}', expected: 'Key', src: {1}", name, src));
        ...
        {
            var resManAcc = new PrivateObject(ResourceManager.Current);
