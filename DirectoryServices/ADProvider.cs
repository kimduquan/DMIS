    ...
    public class ADProvider : DirectoryProvider
    {
        ...
        public static void RetryAllFailedActions()
        {
            using (new SystemAccount())
            {
                ...
                var q = ContentQuery.CreateQuery("TypeIs:File AND InTree:@0 .SORT:CreationDate", qs, _actionPath);
        ...
        {
            var formsADAuthSettings = Configuration.FormsAuthenticationFromADSection.Current.AuthSettings;
            foreach (AuthSettingElement authSetting in formsADAuthSettings)
            {
                if (authSetting.Domain.ToLower() == domain.ToLower())
                {
                    return authSetting;
                }
            }
