...
namespace Tools.IntegrityChecker
{
    internal class Checker : IDisposable
    {
        ...
        {
            const string sql = @"IF (SELECT COUNT(0) FROM [IndexingActivity]) = 0 SELECT 0 ELSE SELECT MAX(IndexingActivityId) FROM IndexingActivity";
            using (var connection = new SqlConnection(GetConnectionString()))
            {
                using (var proc = new SqlCommand())
                {
        ...
        private void CheckIndexState()
        {
            ...
            if (!String.IsNullOrEmpty(gapsInIndex))
                ...
                Console.WriteLine(string.Format("There are tasks to be executed (gap: {0})", gapsInIndex));
            ...
            if (lastIdFromDb != lastIdFromIndex)
                ...
                Console.WriteLine(string.Format("There are tasks to be executed (Last task ids do not match. Db:{0}, index:{1}", lastIdFromDb, lastIdFromIndex));
            else
