...
namespace Backup
{
    ...
    class BackupProgram
    {
        ...
        private static string DbBackupPrefix = "ContentRepository-";

        private static string[] SupportedDataProviders = new[] { "ContentRepository.Storage.Data.SqlClient.SqlProvider" };

        private const int DEFAULTINDEXBACKUPSTARTEDTIMEOUT = 30;
        private const int DEFAULTINDEXBACKUPFINISHEDTIMEOUT = 600;
        ...
        internal static List<string> ArgNames = new List<string>(new string[] { ARG_INDEX, ARG_DATABASE, ARG_FULL, ARG_TARGET, ARG_WEB, ARG_KEEPDAYS, ARG_CONFIRM, ARG_WAIT });
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                Run(backupLevel.Value, backupPath, webFolderPath, keepDays, confirm, waitForAttach);
            }
            catch (Exception e)
            {
                Console.WriteLine();
        ...
        private static bool BackupIndex()
        {
            ...
            DistributedApplication.ClusterChannel.AllowMessageProcessing = true;
        ...
        private static bool BackupDatabase(string newBackupDirectory, string timeSuffix)
        {
            ...
            var bakFilePath = Path.Combine(newBackupDirectory, bakName); // C:\Program Files\Microsoft SQL Server\MSSQL.1\MSSQL\Backup\ContentRepository.bak
