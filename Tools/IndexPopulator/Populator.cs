...
namespace Tools.Lucene.IndexPopulator
{
    class Populator
    {
        private static string CR = Environment.NewLine;
        private static List<string> ArgNames = new List<string>(new string[] { "SOURCE", "INDEX", "ASM", "NOBACKUP", "WAIT" });
        ...
        //   012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789
            CR,
            "Content Repository IndexPopulator tool Usage:", CR,
            "IndexPopulator [-?] [-HELP]", CR,
            "IndexPopulator [-SOURCE <source>] [-INDEX <indexpath>] [-ASM <asm>] [-NOBACKUP]", CR,
            CR,
            "Parameters:", CR,
            "<source>:    Content Repository path as the root (default: /Root)", CR,
            "<asm>:       FileSystem folder containig the required assemblies", CR,
            "             (default: location of IndexPopulator.exe)", CR,
            "<indexpath>: Location of Lucene index directory(default: depends on the ", CR,
            "             configuration)", CR,
            "-NOBACKUP:   Disables the backup of the new index to the database.", CR
        );
        ...
        static void Main(string[] args)
        {
            ...
            bool backup = parameters.ContainsKey("NOBACKUP") ? false : true;
            ...
            Run(repositoryPath, indexPath, asmPath, backup);
        }
        private static void Run(string repositoryPath, string indexPath, string asmPath, bool backup)
        {
            using (var x = ContentRepository.Repository.Start(
                new ContentRepository.RepositoryStartSettings { Console = Console.Out, StartLuceneManager = false, StartWorkflowEngine = false, PluginsPath = asmPath }))
            {
                var y = (ContentRepository.RepositoryInstance)x;
                var z = y.StartupTrace;

                ContentRepository.Storage.Data.RepositoryConfiguration.WorkingMode.Populating = true;

                if (indexPath != null)
                    StorageContext.Search.SetIndexDirectoryPath(indexPath);

                _versionCount = ContentRepository.Storage.Data.DataProvider.GetVersionCount(repositoryPath);
        ...
        {
            string[] names = ContentRepository.Storage.TypeHandler.LoadAssembliesFrom(localBin);
        }
