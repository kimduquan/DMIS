    ...
    public class RegistrationWorkflow : WorkflowHandlerBase
    {
        ...
        private void AssertUserName()
        {
            ...
            if (string.IsNullOrEmpty(uname))
                throw new InvalidOperationException(String.Format(ResourceManager.Current.GetString("RegistrationWorkflow", "CompulsoryField"), USERNAME));

            if (ContentQuery.Query(SafeQueries.UserCountByName, null, uname).Count > 0)
                throw new InvalidOperationException(String.Format(ResourceManager.Current.GetString("RegistrationWorkflow", "UserNameAlreadyExist"), uname));
        }
    }
}
