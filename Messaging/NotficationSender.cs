    ...
    internal class NotificationSender
    {
        ...
        public static void StartMessageProcessing()
        {
            if (!_isSmtpConfigured && !TestMode)
            {
                ...
                Logger.WriteError(Logger.EventId.NotDefined, "Notification sender is unable to work. Reason: smtp server isn't configured.");
                return;
            }
        ...
        internal static bool SignMessagesToSend(string lockId)
        {
            ...
            int rows = 0;
            using (var proc = ContentRepository.Storage.Data.DataProvider.CreateDataProcedure(sql))
            {
        ...
        private static void ProcessMessage(Message message, DataHandler context)
        {
            if (String.IsNullOrEmpty(message.Address))
            {
                ...
                Logger.WriteWarning(Logger.EventId.NotDefined, "Message is not sent. Reason: Address is not provided.");
            ...
            while (true)
            {
                ...
                catch (SmtpFailedRecipientException exception)
                {
                    ...
                    else
                    {
                        ...
                        Logger.WriteException(exception);
                        Logger.WriteWarning(Logger.EventId.NotDefined, "Message is removed from queue.");
                        break;
                    }
                }
            }
