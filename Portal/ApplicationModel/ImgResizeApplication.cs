...
namespace Portal.ApplicationModel
{
    ...
    public class ImgResizeApplication : Application, IHttpHandler
    {
        ...
        protected override void OnModifying(object sender, ContentRepository.Storage.Events.CancellableNodeEventArgs e)
        {
            if (HttpContext.Current != null)
                ReCreateCacheFolder();
        }
            ...
            if (!string.IsNullOrEmpty(imageNodePath))
            {
                ...
                else if (ImageType == "ImageData")
                {
                    if (!String.IsNullOrEmpty(ImageFieldName))
                    {
                        try
                        {
                            var contentImageFieldData = content.Fields[ImageFieldName].GetData() as ContentRepository.Fields.ImageField.ImageFieldData;
