...
namespace Portal.UI.Controls.Captcha
{
    ...
    public class CaptchaControl : FieldControl, INamingContainer, IValidator
    {
        ...
        private string _text = ResourceManager.Current.GetString("Captcha", "CaptchaText");
        private string _font = "";
        private CaptchaImage _captcha = new CaptchaImage();
        private Layout _layoutStyle = Layout.Horizontal;
        private string _prevguid;
        private string _errorMessage = ResourceManager.Current.GetString("Captcha", "WrongAnswer");
        ...
        public void ValidateCaptcha()
        {
            ...
            {
                if ((ci.RenderedAt.AddSeconds(this.CaptchaMinTimeout) > System.DateTime.UtcNow))
                {
                    _userValidated = false;
        ...
        private void GenerateNewCaptcha()
        {
            if (!IsDesignMode)
            {
                if (_cacheStrategy == CacheType.HttpRuntime)
                {
                    HttpRuntime.Cache.Add(_captcha.UniqueId, _captcha, null, DateTime.UtcNow.AddSeconds(Convert.ToDouble((this.CaptchaMaxTimeout == 0 ? 90 : this.CaptchaMaxTimeout))), TimeSpan.Zero, System.Web.Caching.CacheItemPriority.NotRemovable, null);
                }
                else
                {
        ...
        //    return Properties.Resources.ResourceManager.GetString(strResId);
