...
namespace Portal.UI.Controls
{
    ...
    public class ActionLinkButton : System.Web.UI.WebControls.HyperLink, IActionUiAdapter
    {
        ...
        public override void RenderBeginTag(HtmlTextWriter writer)
        {
            ...
            title = string.IsNullOrEmpty(title) ? this.ToolTip : title;

            if (!string.IsNullOrEmpty(SpanClass))
            {
                writer.Write(IconHelper.RenderIconTagWithSpan(SpanClass, title));
            }
            else
            {
                writer.Write(!string.IsNullOrEmpty(IconUrl)
                    ? IconHelper.RenderIconTagFromPath(IconUrl, overlay, IconSize, title)
                    : IconHelper.RenderIconTag(IconName, overlay, IconSize, title));
            }
        }
