...
Type.registerNamespace('Portal.UI.Controls.ActionMenu');

Portal.UI.Controls.ActionMenu = function(element) {
  Portal.UI.Controls.ActionMenu.initializeBase(this, [element]);
}

Portal.UI.Controls.ActionMenu.prototype = {
    ...
    initialize: function () {
        ...
        $actionmenuInner.click(function () {
            ...
            // create div
            $('body > form').append('<div id="' + popupElementId + '" class="sn-actionmenu-dd ui-widget ui-helper-hidden" ><div class="ui-widget-content ui-corner-all"><div id="' + loadingElementId + '" class="sn-actionmenu-loading">' + SN.Resources.ActionMenu["Loading"] + '</div><ul id="' + resultElementId + '" class="ui-helper-reset" style="display:none;"></ul></div></div>');
        ...
        var buildPopup = function (results) {
            ...
            if (results && results.length > 0) {
                ...
                var msoeditenabled = false;
                var ie = false;

                if (navigator.userAgent.match(/Trident\/7\./) || navigator.userAgent.match(/Trident\/6\./)) {
                    ie = true;
                    msoeditenabled = true;
                }

                if (!ie && navigator.mimeTypes && navigator.mimeTypes["application/x-sharepoint"] && navigator.mimeTypes["application/x-sharepoint"].enabledPlugin) {
                    msoeditenabled = true;
                }
                ...
                $.each(results, function () {
                    ...
                    if (!ie && !msoeditenabled && this.Text == SN.Resources.ActionMenu["EditInMicrosoftOffice"]) {
                        link = '<a class="sn-actionlink ui-state-default ui-corner-all sn-disabled ' + this.CssClass + '" disabled="disabled">' + icon + this.Text + '</a>';
                    }
                    else if (this.Callback) {
                        link = '<a href="javascript:void(0);" class="sn-actionlink ui-state-default ui-corner-all ' + this.CssClass + '" onclick="' + this.Callback + '">' + icon + this.Text + '</a>';
                    }
                ...
                $menuItem.append('<li>' + SN.Resources.ActionMenu["NoActionsAvailable"] + '</li>');
            }
...
Portal.UI.Controls.ActionMenu.registerClass("Portal.UI.Controls.ActionMenu", Sys.UI.Control);
