...
using Diagnostics;
using ConfigurationException=ContentRepository.Storage.Data.ConfigurationException;
...
{    
    /// <summary>
    /// <c>FieldControl</c> class is responsible for visulaizing an underlying <see cref="ContentRepository.Field">Field</see> in various <see cref="Portal.UI.Controls.FieldControlRenderMode">FieldControlRenderMode</see>s.
    /// </summary>
    /// <remarks>
    /// It is responsible for retreiving <see cref="ContentRepository.Field">Field</see> data, writing this data back and rendering data set based on the rendering mode given.
    /// 
    /// It is neither responsible for data validation (this is done at <see cref="ContentRepository.Field">Field</see> level) nor for error handling (this is done at <see cref="Portal.UI.ContentView">ContentView</see> level).
    /// However, error visualization is its responsibity.
    /// 
    /// FieldControls are typically created when displaying data in more than one <see cref="Portal.UI.Controls.FieldControlRenderMode">FieldControlRenderMode</see>s. 
    /// If a Field is to be viewed only in Browse mode, this should be donw with public methods and properties provided by <see cref="Portal.UI.ContentView">ContentView</see>.
    /// </remarks>
    [ParseChildren(true)]
    public abstract class FieldControl : ViewControlBase, INamingContainer, IFieldControl 
    {
        ...
        /// <summary>
        /// The <see cref="ContentRepository.Field">Field</see> wrapped by the FieldControl
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the <see cref="ContentRepository.Field">Field</see> property
        /// </summary>
        public Field Field
        {
            get { return _field; }
        ...
        /// <summary>
        /// Gets the <see cref="ContentRepository.Content">Content</see> belonging to the <see cref="ContentRepository.Field">Field</see> property
        /// </summary>
        ...
        /// <summary>
        /// Gets the <see cref="ContentRepository.Storage.Node">Node</see> belonging to the <see cref="ContentRepository.Content">Content</see> owned by the <see cref="ContentRepository.Field">Field</see> property.
        /// </summary>
        ...
        {
            get { return _readOnly || (this.Field == null ? false : this.Field.ReadOnly) || this.ControlMode == FieldControlControlMode.Browse; }
            set { _readOnly = value; }
        }
        ...
        /// </summary>
        /// <param name="data">Data of the <see cref="ContentRepository.Field">Field</see> wrapped</param>
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            {
                Logger.WriteWarning(EventId.FieldControls.InitError, "Error during field control init. " + ex.Message, properties: new Dictionary<string, object>
                                                                                                                         {
                                                                                                                             { "Field", _fieldName },
        ...
        protected void SetDataInternal()
		{
            ...
            if (this.ControlMode == FieldControlControlMode.Edit && _field.IsLocalized && (!_field.Content.IsNew || _field.Content.ContentHandler.Template == null))
                this.SetData(_field.GetStoredValue());
            else
                this.SetData(_field.GetData());
        }
        ...
        protected virtual void InitTemplates()
        {
            ...
            // Elevation: we need access to the field control templates, but
            // regular users (or Visitor) do not have Open permission for them.
