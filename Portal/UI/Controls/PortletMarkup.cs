...
namespace Portal.UI.Controls
{
    public class PortletMarkup
    {
        ...
        public void AddToControls(ControlCollection controls)
        {
            ...
            var portlet = TypeHandler.CreateInstance("Portal.Portlets.ContentCollectionPortlet") as ContextBoundPortlet;
            portlet.ID = Guid.NewGuid().ToString();
            portlet.CustomRootPath = this.CustomRootPath;
            portlet.Renderer = this.Renderer;
            portlet.RenderingMode = Portal.UI.PortletFramework.RenderMode.Xslt;
