    ...
    public partial class PortletSynchronizer : UserControl
    {
        ...
        private void BindList()
        {
            var uninstalled = this.GetUninstalledPortlets().
                Select(p => new
                                {
                                    DisplayName = ResourceManager.Current.GetString(p.Portlet.Name), 
                                    Description = ResourceManager.Current.GetString(p.Portlet.Description), 
                                    Category = ResourceManager.Current.GetString(p.Portlet.Category.Title)
                                }).
