    ...
    public static class WikiTools
    {
            ...
            while (true)
            {
                ...
                using (new SystemAccount())
                {
                    ...
                    var articleQuery = wikiWs == null
                        ? ContentQuery.CreateQuery(SafeQueries.WikiArticlesByDisplayName, wikiQuerySettings, wTitle)
                        : ContentQuery.CreateQuery(SafeQueries.WikiArticlesByDisplayNameAndSubtree, wikiQuerySettings, wTitle, wikiWs.Path);
            ...
            while (true)
            {
                ...
                using (new SystemAccount())
                {
                    if (StorageContext.Search.IsOuterEngineEnabled && StorageContext.Search.SearchEngine != InternalSearchEngine.Instance)
                        targetArticle = ContentQuery.Query(SafeQueries.WikiArticlesByPath,
        ...
        private static void RefreshArticlesAsyncInternal(WikiArticle targetArticle, WikiArticleAction articleAction, string oldName, string oldDisplayName)
        {
            ...
            using (new SystemAccount())
            {
                ...
                results = ContentQuery.Query(SafeQueries.WikiArticlesByReferenceTitlesAndSubtree,
