...
namespace Portal.Handlers
{
    public class UploadHelper
    {
        ...
        private static NameValueCollection FileExtensions
        {
            get
            {
                return System.Configuration.ConfigurationManager.GetSection("dmis/uploadFileExtensions") as NameValueCollection;
            }
        }
        ...
        public static string GetContentType(string fileName, string contextPath)
        {
            ...
            var ctName = Settings.GetValue(PortalSettings.SETTINGSNAME, PortalSettings.SETTINGS_UPLOADFILEEXTENSIONS + extension, contextPath, string.Empty);
            ...
            var execExt = Tools.IsExecutableExtension(extension);

            if (!execExt)
            {
                //look for the default setting
                ctName = Settings.GetValue(PortalSettings.SETTINGSNAME, PortalSettings.SETTINGS_UPLOADFILEEXTENSIONS_DEFAULT, contextPath, string.Empty);
                if (!string.IsNullOrEmpty(ctName))
                    return ctName;
            }
