            ...
            try
            {
                ...
                foreach (var content in ExportContent.Children)
                {
                    ...
                    foreach (var fieldName in header)
                    {
                        ...
                        {
                            Logger.WriteWarning(Logger.EventId.NotDefined, string.Format("Error during field CSV export. Content: {0}. Field: {1}. User: {2}.{3}Error: {4}", content.Path, fieldName, User.Current.Username, Environment.NewLine, ex.ToString()));
                            result.Append("\"\"");
                        }
        ...
        private List<string> GetVisibleFieldNames()
        {
            ...
            foreach (var fieldSetting in leafFieldSettings)
            {
                ...
                while (fs != null)
                {
                    ...
                    //only leaf fields are checked, we do not make a parentwalk here
                    //to avoid adding all visible fields on GenericContent
