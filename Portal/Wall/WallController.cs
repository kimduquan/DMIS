...
namespace Portal.Wall
{
    public class WallController : Controller
    {
        ...
        /// <returns></returns>
        [AcceptVerbs(HttpVerbs.Get)]
        public ActionResult Unlike(string itemId, string contextPath, bool fullMarkup, string rnd)
        {
            AssertPermission();
        ...
        private void SetCurrentWorkspace(string workspacePath)
        {
            PortalContext.ContextWorkspaceResolver = new Func<PortalContext, Workspace>(p => Node.LoadNode(workspacePath) as Workspace);
        }
    }
}
