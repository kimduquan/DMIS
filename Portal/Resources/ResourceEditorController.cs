...
namespace Portal
{
    public class ResourceEditorController : Controller
    {
        ...
        public ActionResult GetAllResourcesForClass(string classname, string rnd)
        {
            ...
            foreach (var name in ResourceManager.Current.GetClassKeys(classname).OrderBy(x => x))
            {
                ...
                foreach (var lang in langs)
                {
                    try
                    {
                        ...
                        var s = ResourceManager.Current.GetObjectOrNull(classname, name, cultureInfo) as string;
        ...
        internal static Dictionary<string, string> GetResources(string classname, string name)
        {
            ...
            foreach (var lang in langs)
            {
                ...
                {
                    var cultureInfo = new CultureInfo(lang);
                    var s = ResourceManager.Current.GetObjectOrNull(classname, name, cultureInfo, false, false) as string;
                    resources.Add(lang, s);
                }
                catch (Exception ex)
                {
        ...
        public static IEnumerable<string> GetCurrentSupportedLanguageNames()
        {
            var languages = Portal.Site.GetAllLanguages()
                ...
                .Concat(ResourceManager.Current.GetCultures().Select(x => x.Name))
                .Distinct()
                .ToList();
        ...
        private static string GetOptionLocalizedText(IEnumerable<ChoiceOption> options, string langKey)
        {
            ...
            string className;
            string name;
            if (ResourceManager.ParseResourceKey(text, out className, out name))
            {
                var localizedText = ResourceManager.Current.GetObjectOrNull(className, name, CultureInfo.CurrentUICulture, false) as string;
        ...
        public static void InitEditorScript(System.Web.UI.Page page)
        {
            ...
            var siteContent = ContentRepository.Content.Create(PortalContext.Current.Site);
