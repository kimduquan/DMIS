...
namespace Portal.Resources
{
    public static class ResourceScripter
    {
        public static string RenderResourceScript(string className, CultureInfo cultureInfo)
        {
            ...
            var values = ResourceManager.Current.GetClassItems(className, cultureInfo);
            ...
            if ((values == null || values.Count == 0) && !cultureInfo.IsNeutralCulture)
            {
                ...
                values = ResourceManager.Current.GetClassItems(className, cultureInfo);
            }

            // use some or all of the strings defined for the fallback culture if they are not defined for the main culture
            if (!string.IsNullOrWhiteSpace(ResourceManager.FallbackCulture))
            {
                cultureInfo = CultureInfo.GetCultureInfo(ResourceManager.FallbackCulture);
                var fallbackValues = ResourceManager.Current.GetClassItems(className, cultureInfo);
