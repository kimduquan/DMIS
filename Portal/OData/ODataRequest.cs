...
namespace Portal.OData
{
    ...
    internal enum InlineCount { None, AllPages }
    internal enum MetadataFormat {None, Minimal, Full}
    ...
    internal class ODataRequest
    {
        ...
        internal const string ODATA_SVC = "OData.svc"; // url segment
        ...
        internal static string GetContentPathFromODataRequest(string requestPath)
        {
            ...
            if (!requestPath.StartsWith(ContentRepository.Storage.RepositoryPath.PathSeparator + ODATA_SVC, StringComparison.OrdinalIgnoreCase))
                return string.Empty;

            // cut odata.svc from the beginning
            var path = requestPath.Substring(ODATA_SVC.Length + 1);
