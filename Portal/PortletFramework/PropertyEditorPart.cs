...
namespace Portal.UI.PortletFramework
{
    public class PropertyEditorPart : EditorPart
    {
        ...
        public static string GetCategoryTitle(PropertyDescriptor propertyDescriptor)
        {
            ...
            if (category == null)
            {
                // treat it as Other Category
                return ResourceManager.Current.GetString("PortletFramework", "EditorCategory_Other");
            }

            return String.IsNullOrEmpty(category.Title) ? string.Empty : ResourceManager.Current.GetString(category.Title);
        }
