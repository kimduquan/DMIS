...
[assembly: TagPrefix("Portal.UI.PortletFramework", "sn")]
namespace Portal.UI.PortletFramework
{
    ...
    public class SNWebPartChrome : WebPartChrome
    {
        ...
        public override void RenderWebPart(HtmlTextWriter writer, WebPart webPart)
        {
            ...
            //RenderReferences(writer, webPart);
        ...
        //private void RenderReferences(HtmlTextWriter writer, WebPart webPart)
        //{
        //    if (!User.Current.IsInGroup(Group.Administrators))
        //        return;

        //    var portlet = webPart as PortletBase;

        //    // get properties with WebBrowsable(true) attribute
        //    var props = portlet.GetType().GetProperties().Where(p => Attribute.IsDefined(p, typeof(WebBrowsableAttribute)) && ((WebBrowsableAttribute)p.GetCustomAttributes(true).First(a => a is WebBrowsableAttribute)).Browsable);

        //    // add all referenced paths
        //    var references = new List<Tuple<string, string>>();
        //    foreach (PropertyInfo prop in props)
        //    {
        //        var propValue = prop.GetValue(portlet, null) as string;
        //        if (propValue == null)
        //            continue;

        //        if (propValue.StartsWith("/Root"))
        //            references.Add(new Tuple<string, string>(prop.Name, propValue));
        //    }
        ...
        //    {
        //        if (contextboundPortlet.ContextNode != null)
        //            references.Add(new Tuple<string, string>("Context", contextboundPortlet.ContextNode.Path));
        //        else
        //            references.Add(new Tuple<string, string>("Context", "-"));

        //        references.RemoveAll(t => t.Item1 == "CustomRootPath"); // remove customrootpath
        //    }

        //    if (references.Count > 0)
        //    {
        //        writer.AddAttribute(HtmlTextWriterAttribute.Class, "sn-portlet-references");
        //        writer.AddAttribute(HtmlTextWriterAttribute.Style, "background-color:#FFF;color:#000;border:1px solid #AAA;padding:3px;margin-bottom:2px;display:none;");
        //        writer.RenderBeginTag(HtmlTextWriterTag.Div);

        //        foreach (var reference in references)
        //        {
        //            writer.Write("<div>" + reference.Item1 + ": <a href=\""+reference.Item2+"?action=Edit\" target=\"blank\">" + reference.Item2 + "</a></div>");
        //        }

        //        writer.RenderEndTag();

        //        writer.AddAttribute(HtmlTextWriterAttribute.Style, "background-color:#007DC2;color:#FFF;position:absolute;z-index:1000;font-weight:bold;padding:0 2px 0 2px;cursor:default;");
        //        writer.AddAttribute(HtmlTextWriterAttribute.Onclick, "$('.sn-portlet-references',$(this).closest('.sn-portlet')).toggle();");
        //        writer.AddAttribute(HtmlTextWriterAttribute.Title, "Show referenced content");
        //        writer.RenderBeginTag(HtmlTextWriterTag.Div);
        //        writer.Write("R");
        //        writer.RenderEndTag();
        //    }
        ...
        private static string GetTitleByFormat(string title, WebPart wp)
        {
            ...
            //try
            //{
            //    var isExpression = title.Contains(ContentRepository.i18n.ResourceManager.ResourceStartKey);
            //    if (isExpression)
            //    {
            //        var resourceTitle = ContentRepository.i18n.ResourceManager.Current.GetStringByExpression(title);
            ...
            try
            {
                ...
                if (title.Contains(ContentRepository.i18n.ResourceManager.ResourceStartKey))
                    title = ContentRepository.i18n.ResourceManager.Current.GetStringByExpression(title);
